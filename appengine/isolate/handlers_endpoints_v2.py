...
@auth.endpoints_api(
    name='isolateservice', version='v2',
    description='Version 2 of Isolate Service')
class IsolateServiceV2(remote.Service):
    ...
    for index, digest_element in enumerate(request.items):
      ...
      if digest_element in new_digests:
        ...
        if self.should_push_to_gs(digest_element):
          key = entry_key_or_error(request.namespace, digest_element.digest)
  ...
  def retrieve(self, request):
    ...
    memcache_entry = memcache.get(
        request.digest, namespace='table_%s' % request.namespace)
    ...
    # try ndb
      ...
      key = entry_key_or_error(request.namespace, request.digest)
      stored = key.get()
      if stored is None:
  ...
  def check_entries_exist(cls, entries):
    ...
    for digest in entries.items:
      # check for error conditions
      key = entry_key_or_error(entries.namespace, digest.digest)
