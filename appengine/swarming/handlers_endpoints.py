...
def _decode_field(self, field, value):
  if isinstance(field, messages.BooleanField) and isinstance(value, basestring):
...
class SwarmingTaskService(remote.Service):
  ...
  def cancel(self, request):
    ...
    ok, was_running = api_common.cancel_task(
  ...
  def stdout(self, request):
    ...
    return swarming_rpcs.TaskOutput(
      output=output, state=swarming_rpcs.TaskState(state)
    )
...
class SwarmingTasksService(remote.Service):
  ...
  def new(self, request):
    ...
    return swarming_rpcs.TaskRequestMetadata(
      ...
      task_id=ntr.task_id,
      task_result=message_conversion.task_result_to_rpc(ntr.task_result, False)
      if ntr.task_result
      else None,
    )
  ...
  def cancel(self, request):
    ...
    return swarming_rpcs.TasksCancelResponse(
      cursor=tcr.cursor, matched=tcr.matched, now=tcr.now
    )
          ...
          for dims_flat, valid_until_ts in i.expiry_map().items():
            ...
            out.append(
              swarming_rpcs.TaskQueue(
                dimensions=list(dims_flat), valid_until_ts=valid_until_ts
              )
            )
        count += 1
    except ValueError as e:
...
class SwarmingBotService(remote.Service):
  ...
  @endpoint(
    BotId, swarming_rpcs.DeletedResponse, name="delete", path="{bot_id}/delete"
  )
  @auth.require(acl.can_access, log_identity=True)
...
BotsCountRequest = endpoints.ResourceContainer(
  message_types.VoidMessage, dimensions=messages.StringField(1, repeated=True)
)
