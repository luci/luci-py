...
class _BotAuthenticatingHandler(auth.AuthenticatingHandler):
  ...
  def check_bot_code_access(self, bot_id, generate_token):
    ...
    if (
      not acl.can_create_bot()
      and not (bot_id and bot_auth.is_authenticated_bot(bot_id))
      and not acl.is_ip_whitelisted_machine()
    ):
...
class BotCodeHandler(_BotAuthenticatingHandler):
  ...
  def get(self, version=None):
    ...
    known = version and version in (
      info.stable_bot.digest,
    ...
    if not version or not known:
      ...
      bot_id = (
        self.request.get("bot_id")
        or self.request.headers.get(self._X_LUCI_SWARMING_BOT_ID)
        or _bot_id_from_auth_token()
      )
  ...
  def redirect_to_version(self, version):
    self.redirect(
      "%s/swarming/api/v1/bot/bot_code/%s"
...
def _validate_dimensions(dimensions):
  ...
  for key, values in sorted(dimensions.items()):
    ...
    if not isinstance(values, list):
      error_msgs.append(
        "Dimension values must be list. key: %s, values: %s" % (key, values)
      )
    ...
    for value in sorted(values):
      if config.validate_dimension_value(value):
        continue
      error_msgs.append(
        "Invalid dimension value. key: %s, value: %s" % (key, value)
      )
...
class _BotBaseHandler(_BotApiHandler):
  ...
  EXPECTED_KEYS = {"dimensions", "state", "version"}
  OPTIONAL_KEYS = {"session"}
  REQUIRED_STATE_KEYS = {"running_time", "sleep_streak"}
  ...
  def process(self):
    ...
    if dimensions.get("id"):
      ...
      if (
        isinstance(dimension_id, list)
        and len(dimension_id) == 1
    ...
    bot_details = task_scheduler.BotDetails(
      version, bot_group_cfg.logs_cloud_project
    )
    ...
    err_msg = has_unexpected_subset_keys(
      self.EXPECTED_KEYS | self.OPTIONAL_KEYS,
      self.EXPECTED_KEYS,
      request,
      "keys",
    )
  ...
  def prepare_manifest(
    self, request, secret_bytes, run_result, bot_request_info
  ):
    ...
    if request.resultdb_update_token:
      resultdb_context = {
        "hostname": urlparse.urlparse(
          config.settings().resultdb.server
        ).hostname,
        "current_invocation": {
          "name": resultdb.get_invocation_name(
            task_pack.pack_run_result_key(run_result.run_result_key)
          ),
          "update_token": request.resultdb_update_token,
        },
      }
    realm_context = {}
    if request.realm:
    ...
    out = {
      "bot_id": bot_request_info.bot_id,
      ...
      if props.cas_input_root and props.cas_input_root.cas_instance
      else None,
      ...
      "service_accounts": {
        "system": {
          ...
          "service_account": bot_request_info.bot_group_cfg.system_service_account
          or "none",
        },
        "task": {
          # Same here.
      ...
      "bot_dimensions": bot_request_info.dimensions,
    }
...
class BotPollHandler(_BotBaseHandler):
  ...
  OPTIONAL_KEYS = {"session", "request_uuid", "force"}
  ...
  def post(self):
    ...
    if res.rbe_effective_bot_id_dimension:
      ...
      # constraint.
      assert len(pools) == 1, (
        "bot using rbe_effective_bot_id must belong to only one pool"
      )
      pool = pools[0]

      effective_bot_ids = res.dimensions.get(
    ...
    if res.bot_id != session.bot_id:
      ...
      self.abort_with_error(
        403, error="Unexpected bot ID in the session %s" % session.bot_id
      )
      return
    logging.info("Session ID: %s", session.session_id)
    ...
    session_token = bot_session.marshal(
      bot_session.update(
        ...
        res.rbe_effective_bot_id_dimension if rbe_effective_bot_id else None,
      )
    )
    ...
    if bot_session.is_stale_handshake_config(session, res.bot_group_cfg):
      bot_event("request_restart")
      self._cmd_bot_restart(
        session_token, "Restarting to pick up new bots.cfg config"
      )
      return
    ...
    if bot_info and bot_info.task_id:
      logging.error(
        "Task %s is already assigned to the bot %s",
        bot_info.task_id,
        res.bot_id,
      )
    ...
    if scheduler_use:
      ...
      try:
        (request, secret_bytes, run_result), is_deduped = (
          api_helpers.cache_request(
            "bot_poll",
            request_uuid,
            lambda: task_scheduler.bot_reap_task(
    ...
    if request.task_slice(
      ...
      bot_event(
        "request_task", task_id=run_result.task_id, task_name=request.name
      )
      self._cmd_run(session_token, request, secret_bytes, run_result, res)

  def _cmd_run(
    self, session, request, secret_bytes, run_result, bot_request_info
  ):
    logging.info("Run: %s", request.task_id)
    manifest = self.prepare_manifest(
      request, secret_bytes, run_result, bot_request_info
    )
    out = {
      "cmd": "run",
      "manifest": manifest,
...
class BotClaimHandler(_BotBaseHandler):
  ...
  EXPECTED_KEYS = _BotBaseHandler.EXPECTED_KEYS | {
    ...
    "task_id",  # TaskResultSummary packed ID
    "task_to_run_shard",  # shard index identifying TaskToRunShardXXX class
    "task_to_run_id",  # TaskToRunShardXXX integer entity ID
  }
...
class BotEventHandler(_BotBaseHandler):
  ...
  EXPECTED_KEYS = _BotBaseHandler.EXPECTED_KEYS | {"event", "message"}
    ...
    try:
      if account_id == "task":
        account, token = service_accounts.get_task_account_token(
          task_id, bot_id, self.TOKEN_KIND, scopes=scopes, audience=audience
        )
...
class BotOAuthTokenHandler(_BotTokenHandler):
  ...
  ACCEPTED_KEYS = {
    "account_id",  # 'system' or 'task'
    "id",  # bot ID
    "scopes",  # list of requested OAuth scopes
    "task_id",  # optional task ID, required if using 'task' account
    "session",  # the session token
  }
  REQUIRED_KEYS = {"account_id", "id", "scopes"}
...
class BotIDTokenHandler(_BotTokenHandler):
  ...
  ACCEPTED_KEYS = {
    "account_id",  # 'system' or 'task'
    "id",  # bot ID
    "audience",  # the string audience to put into the token
    "task_id",  # optional task ID, required if using 'task' account
    "session",  # the session token
  }
  REQUIRED_KEYS = {"account_id", "id", "audience"}
  ...
  def post(self, task_id=None):
    ...
    if cipd_pins:
      cipd_pins = task_result.CipdPins(
        client_package=task_request.CipdPackage(**cipd_pins["client_package"]),
        packages=[
...
def get_routes():
  ...
  add(
    "/swarming/api/v1/bot/task_update/<task_id:[a-f0-9]+>", BotTaskUpdateHandler
  )
