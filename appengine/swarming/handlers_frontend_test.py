...
class FrontendTest(test_env_handlers.AppTestBase):
  ...
  def test_all_swarming_handlers_secured(self):
    ...
    routes_to_check = [
      ...
      for route in routes
      if (
        route.template not in public_urls
        and not route.template.startswith(using_app_login_prefixes)
      )
    ]
    ...
    def check_protected(route, method):
      ...
      body = ""
      if method == "POST" and (
        path.startswith("/swarming/api/v1/bot/")
        or path.startswith("/python/swarming/api/v1/bot/")
      ):
      ...
      message = "%s handler is not protected: %s, returned %s" % (
        method,
        path,
        response,
      )
