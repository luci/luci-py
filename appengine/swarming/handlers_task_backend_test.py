...
class TaskBackendAPIServiceTest(test_env_handlers.AppTestBase):
  ...
  def _basic_run_task_request(self):
    return backend_pb2.RunTaskRequest(
      ...
      backend_config=struct_pb2.Struct(
        fields={
          "priority": struct_pb2.Value(number_value=1),
  ...
  def test_run_task_exceptions_schedule_request_error(self):
    ...
    def mocked_schedule_request(
      _, start_time=0, secret_bytes=None, build_task=None
    ):
  ...
  def test_cancel_tasks(self, mocked_enqueue_task):
    ...
    _, second_id = self.client_create_task_raw(
      name="second", user="jack@localhost", tags=["project:yay", "commit:pre"]
    )

    self.set_as_project()
    request = backend_pb2.CancelTasksRequest(
      task_ids=[
    ...
    expected_response = backend_pb2.CancelTasksResponse(
      tasks=[
        task_pb2.Task(
  ...
  def test_cancel_tasks_too_many(self):
    ...
    self.assertEqual(
      raw_resp.body,
      ("Requesting 3 tasks for cancellation when the allowed max is 2."),
    )
  ...
  def test_fetch_tasks(self):
    ...
    expected_response = backend_pb2.FetchTasksResponse(
      responses=[
        backend_pb2.FetchTasksResponse.Response(
          task=task_pb2.Task(
            id=task_pb2.TaskID(target=target, id=first_id),
            status=common_pb2.SCHEDULED,
            update_id=1546398020,
  ...
  def test_fetch_tasks_too_many(self):
    ...
    self.assertIn(("X-Prpc-Grpc-Code", "3"), raw_resp._headerlist)
    self.assertEqual(
      raw_resp.body, "Requesting 3 tasks when the allowed max is 2."
    )
  ...
  def test_validate_configs(self):
      ...
      configs=[
        backend_pb2.ValidateConfigsRequest.ConfigContext(
          ...
          config_json=struct_pb2.Struct(
            fields={
              "bot_ping_tolerance": struct_pb2.Value(
                number_value=task_request._MAX_BOT_PING_TOLERANCE_SECS
              ),
            }
          ),
        )
    ...
    self.assertEqual(
      resp, backend_pb2.ValidateConfigsResponse(config_errors=[])
    )
