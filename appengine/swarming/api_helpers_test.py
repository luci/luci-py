...
class TestProcessTaskRequest(test_case.TestCase):
      ...
      task_slices=[
        task_request.TaskSlice(
          ...
          properties=task_request.TaskProperties(
            ...
            dimensions_data={"chicken": ["egg1", "egg2"], "pool": ["default"]},
          ),
        )
      ],
    )
  ...
  def test_process_task_request_BadRequest(self):
    tr = task_request.TaskRequest(
      ...
      task_slices=[
        task_request.TaskSlice(
          properties=task_request.TaskProperties(
            dimensions_data={"chicken": ["egg1", "egg2"]}
          )
        )
      ],
    )
    ...
    with self.assertRaisesRegexp(
      handlers_exceptions.BadRequestException, "reserved for internal use"
    ):
    ...
    with self.assertRaisesRegexp(
      handlers_exceptions.BadRequestException, "missing expiration_secs"
    ):
      api_helpers.process_task_request(tr, task_request.TEMPLATE_AUTO)
    ...
    with self.assertRaisesRegexp(
      handlers_exceptions.BadRequestException, "name is missing"
    ):
      api_helpers.process_task_request(tr, task_request.TEMPLATE_AUTO)
    ...
    # Catch no such pool.
    with self.assertRaisesRegexp(
      handlers_exceptions.PermissionException, "No such pool"
    ):
      api_helpers.process_task_request(tr, task_request.TEMPLATE_AUTO)
    ...
    self.mock_pool_config("default", informational_dimensions_re=r"label-*")
  ...
  def test_process_task_request_service_account_legacy(self):
    ...
    task_request.init_new_request(
      tr, acl.can_schedule_high_priority_tasks(), task_request.TEMPLATE_AUTO
    )
  ...
  def test_process_task_request_rbe_mode(self):
    ...
    self.assertIn("rbe:rbe-inst", tr.tags)

  def mock_pool_config(
    self, name, rbe_migration=None, informational_dimensions_re=None
  ):
...
class TestCheckIdenticalRequest(test_case.TestCase):
  ...
  def test_cache_hit(self):
    ...
    self.assertEqual(
      ("ok", False),
      api_helpers.cache_request("test_request", request_uuid, func),
    )
  ...
  def test_namespace(self):
    ...
    self.assertEqual(
      ("ok", False),
      api_helpers.cache_request("test_request_1", request_uuid, func1),
    )
    ...
    self.assertEqual(
      ("great", False),
      api_helpers.cache_request("test_request_2", request_uuid, func2),
    )
  ...
  def test_validate_configs(self):
    configs = [
      ...
      swarming_pb2.SwarmingTaskBackendConfig(priority=0, bot_ping_tolerance=1),
    ]
    ...
    expected_errors = [
      ...
      (
        0,
        (
          'service_account must be an email, "bot" or "none"'
          " string, got u'bokbok'"
        ),
      ),
  ...
  def test_validate_configs_full_validation(self):
    ...
    expected_errors = [
      ...
      (
        0,
        (
          "agent_binary_cipd_pkg must be a valid CIPD package name"
