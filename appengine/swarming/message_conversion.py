  ...
  if props.cipd_input:
    ...
    if props.cipd_input.client_package:
      client_package = _ndb_to_rpc(
        swarming_rpcs.CipdPackage, props.cipd_input.client_package
      )
...
def _taskslice_from_rpc(msg):
  ...
  out = _rpc_to_ndb(
    task_request.TaskSlice, msg, properties=props, properties_hash=None
  )
  return out, secret_bytes
...
def epoch_to_datetime(value):
  ...
  if not value:
    return None
  try:
    return utils.timestamp_to_datetime(value * 1000000.0)
...
def task_result_to_rpc(entity, send_stats):
  ...
  if entity.cipd_pins:
    cipd_pins = swarming_rpcs.CipdPins(
      client_package=(
        _ndb_to_rpc(swarming_rpcs.CipdPackage, entity.cipd_pins.client_package)
        if entity.cipd_pins.client_package
        else None
      ),
      packages=[
  ...
  if entity.missing_cipd:
    missing_cipd = [
      _ndb_to_rpc(swarming_rpcs.CipdPackage, pkg) for pkg in entity.missing_cipd
    ]
