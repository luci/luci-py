...
def _bot_event(event_type, bot_id, **kwargs):
  args = {
    ...
    "dimensions": {"id": [bot_id], "pool": ["default"]},
    "external_ip": "8.8.4.4",
    "state": {"ram": 65},
    "version": "123456789",
    "quarantined": False,
...
class ServerApiTest(BaseTest):
  ...
  def test_details(self):
    ...
    expected = {
      "bot_version": unicode(bot_code.get_bot_version(bot_code.STABLE_BOT)[0]),
      "display_server_url_template": "",
      "luci_config": "a.server",
      "server_version": unicode(utils.get_app_version()),
  ...
  def _test_file(self, name, header):
    ...
    self.mock(
      bot_code.config, "config_service_hostname", config_service_hostname_mock
    )
...
class TasksApiTest(BaseTest):
  ...
  def test_new_ok_raw(self):
    ...
    task_id = expected["request"].pop("task_id")
    ...
    expected["request"]["task_id"] = task_id
    expected["task_result"] = {
      "abandoned_ts": "2010-01-02T03:04:05",
      "completed_ts": "2010-01-02T03:04:05",
      "created_ts": "2010-01-02T03:04:05",
      "current_task_slice": "0",
      "failure": False,
      "internal_failure": False,
      "modified_ts": "2010-01-02T03:04:05",
      "name": "job1",
      "server_versions": ["v1a"],
      "state": "NO_RESOURCE",
      "tags": [
  ...
  def test_new_ok_template(self):
    ...
    props = self.create_props(
      ...
      dimensions=[
        {"key": "os", "value": "Amiga"},
        {"key": "pool", "value": "template"},
      ],
    )

    # We want to observe the pool default
    props["cipd_input"]["client_package"] = None
    props["cipd_input"]["server"] = None

    request = self.create_new_request(properties=props, tags=["a:tag"])
    expected_props = self.gen_props(
      command=["rm", "-rf", "/"],
      execution_timeout_secs="30",
      grace_period_secs="15",
      env=[{"key": "VAR", "value": "prod"}],
      dimensions=[
        {"key": "os", "value": "Amiga"},
        {"key": "pool", "value": "template"},
      ],
    )
    expected_props["cipd_input"]["client_package"] = {
      "package_name": "cipd-client-pkg",
      "version": "from_pool_config",
    }
    expected_props["cipd_input"]["server"] = (
      "https://pool.config.cipd.example.com"
    )
    # Need to add the additional package that the template applies.
    expected_props["cipd_input"]["packages"].insert(
      0, {"package_name": "some-pkg", "path": ".", "version": "prod-version"}
    )
  ...
  def test_new_ok_template_no_additional_packages(self):
    ...
    expected_cipd_props = {
      ...
      "packages": [
        {"path": ".", "version": "prod-version", "package_name": "some-pkg"}
      ],
    }
    ...
    self.assertEqual(
      expected_cipd_props, response.json["request"]["properties"]["cipd_input"]
    )
  ...
  def test_new_bad_service_account(self):
    ...
    self.assertEqual(
      {
        "error": {
          "message": 'service_account must be an email, "bot" or '
  ...
  def test_new_ok_deduped(self):
    ...
    request = handlers_endpoints.TasksRequest.combined_message_class(
      state=swarming_rpcs.TaskStateQuery.DEDUPED, include_performance_stats=True
    )
    response = self.call_api("list", body=message_to_dict(request))
    self.assertEqual(expected, response.json)

    # Use the occasion to test 'count' and 'requests'.
    start = utils.datetime_to_timestamp(self.now) / 1000000.0 - 1
    end = utils.datetime_to_timestamp(now_30) / 1000000.0 + 1
    request = handlers_endpoints.TasksRequest.combined_message_class(
      start=start, end=end, state=swarming_rpcs.TaskStateQuery.DEDUPED
    )
    response = self.call_api("count", body=message_to_dict(request))
    self.assertEqual({"now": fmtdate(now_30), "count": "1"}, response.json)

    expected = {"items": [deduped_request, t_request], "now": fmtdate(now_30)}
    request = handlers_endpoints.TasksRequest.combined_message_class(
      start=start, end=end
    )
    response = self.call_api("requests", body=message_to_dict(request))
  ...
  def test_new_task_slices_one(self):
    ...
    task_slices = [
      {
        "expiration_secs": 180,
        "properties": self.create_props(command=["python", "run_test.py"]),
        "wait_for_capacity": False,
      },
    ]
    ...
    expected_props = self.gen_props(command=["python", "run_test.py"])
    expected = {
      "request": self.gen_request(
        created_ts=fmtdate(self.now),
        expiration_secs="180",
  ...
  def test_new_task_slices_two(self):
    ...
    expected_props_1 = self.gen_props(command=["python", "run_test.py"])
  ...
  def test_new_task_slices_two_denied(self):
    ...
    expected = {
      "error": {
        "message": "expiration_secs (0) must be between 1s and 604800s",
      },
    }
  ...
  def test_new_idempotent(self):
    ...
    with mock.patch(
      ...
      wraps=task_scheduler.schedule_request,
    ) as mock_schedule_request:
      response = self.call_api("new", body=message_to_dict(request))
      # Time advanced since the evaluate_only call.
      expected["request"]["created_ts"] = fmtdate(self.now)
      self.assertEqual(expected, response.json)
  ...
  def test_new_denied_pool(self):
    ...
    # is returned.
    request = self.create_new_request(
      properties={"dimensions": [{"key": "id", "value": "bot123"}]}
    )
    response = self.call_api("new", body=message_to_dict(request), status=400)
    expected = {
      "error": {
        "message": "'pool' must be used as dimensions",
      },
    }
    self.assertEqual(expected, response.json)
  ...
  def test_new_denied_execution_timeout_secs(self):
    ...
    response = self.call_api("new", body=message_to_dict(request), status=400)
    expected = {
      "error": {"message": "'execution_timeout_secs' must be specified"},
    }
    self.assertEqual(expected, response.json)
  ...
  def test_new_ok(self):
    ...
    self.assertEqual("5cee488008810", response.json["task_id"])
  ...
  def test_new_ok_in_realms_mode(self):
    ...
    key, _ = task_pack.get_request_and_result_keys(response.json["task_id"])
    req = key.get()
  ...
  def test_new_invalid_realm(self):
    request = self.create_new_request(
      ...
      service_account="service-account@example.com",
      realm="test/invalid",
    )  # should be <project>:<realm>
    response = self.call_api("new", body=message_to_dict(request), status=400)
    self.assertEqual(
      {
        "error": {
          "message": "Bad realm u'test/invalid', want \"<project>:<name>\"",
        },
      },
      response.json,
    )
  ...
  def test_new_ok_with_resultdb_and_realm(self):
    ...
    with mock.patch(
      "server.resultdb.create_invocation_async", side_effect=self._updateToken
    ):
    ...
    self.assertEqual(
      "invocations/task-test-swarming.appspot.com-5cee488008811",
  ...
  def test_mass_cancel_pending(self):
    ...
    response = self.call_api(
      ...
      body={
        "tags": ["project:yay"],
        "start": utils.datetime_to_timestamp(self.now) / 1000000.0 - 1,
        "end": utils.datetime_to_timestamp(now_120) / 1000000.0 + 1,
      },
    )
    expected = {
      "matched": "1",
  ...
  def test_mass_cancel_realm_permission(self):
    ...
    self.assertErrorResponseMessage("No pool is specified", response)
  ...
  def test_list_ok(self):
    ...
    expected = {"now": fmtdate(now_120), "items": [second, first]}
    ...
    actual = self.call_api("list", body=message_to_dict(request)).json
    self.assertEqual(
      {"now": fmtdate(now_120), "items": [second, first_no_perf]}, actual
    )
    ...
    self.assertEqual(
      {"now": fmtdate(now_120), "items": [second]},
    ...
    self.assertEqual(
      {"now": fmtdate(now_120), "items": [second, first_no_perf]},
      self.call_api("list", body=message_to_dict(request)).json,
    )
  ...
  def test_get_state_ok(self):
    ...
    expected = {"states": ["COMPLETED", "PENDING", "PENDING"]}
              ...
              if (using_filter, state, sort) in unspported or (
                ...
                try:
                  result = self.call_api(
                    "list", body=message_to_dict(request)
                  ).json
                except:  # pylint: disable=bare-except
                  self.fail(
                    "Is unsupported: (%s, %s, %s)" % (using_filter, state, sort)
                  )
...
class TaskApiTest(BaseTest):
  ...
  def test_cancel_pending(self):
    ...
    expected = {"ok": True, "was_running": False}
    response = self.call_api(
  ...
  def test_cancel_with_realm_permission(self):
    ...
    def assertTaskIsNotAccessible(task_id):
      ...
      self.assertErrorResponseMessage(
        'Task "%s" is not accessible' % task_id, response
      )
  ...
  def test_cancel_running(self):
    ...
    self.assertEqual({"ok": False, "was_running": True}, response.json)

    # Works if kill_running == True.
    response = self.call_api(
      "cancel", body={"task_id": task_id, "kill_running": True}
    )
    self.assertEqual({"ok": True, "was_running": True}, response.json)

    self.set_as_bot()
    params = _params(output=base64.b64encode("hi"), output_chunk_start=3)
    response = self.post_json("/swarming/api/v1/bot/task_update", params)
    self.assertEqual({"must_stop": True, "ok": True}, response)
  ...
  def test_result_completed_task(self):
    ...
    expected["performance_stats"] = self.gen_perf_stats()
    response = self.call_api(
      "result", body={"task_id": task_id, "include_performance_stats": True}
    )
  ...
  def test_stdout_ok(self):
    ...
    self.set_as_privileged_user()
    run_id = task_id[:-1] + "1"
    expected = {"output": "rÉsult string", "state": "COMPLETED"}
    ...
    expected = {"output": "É", "state": "COMPLETED"}
    self.assertEqual(expected, response.json)
    req = {"task_id": task_id, "offset": 3, "length": 5}
    response = self.call_api("stdout", body=req)
    expected = {"output": "sult ", "state": "COMPLETED"}
    self.assertEqual(expected, response.json)
  ...
  def test_stdout_empty(self):
    ...
    self.assertEqual({"state": "NO_RESOURCE"}, response.json)
  ...
  def test_request_ok(self):
    ...
    expected_props = self.gen_props(
      command=["python", "run_test.py"], secret_bytes="PFJFREFDVEVEPg=="
    )  # <REDACTED> in base64
...
class QueuesApiTest(BaseTest):
  ...
  def test_list(self):
    ...
    expected = {
      "items": [
        {
          "dimensions": ["id:bot123", "pool:default"],
          "valid_until_ts": "2010-01-03T07:19:07",
        },
        {
          "dimensions": ["os:Amiga", "pool:default"],
          "valid_until_ts": "2010-01-03T07:19:05",
        },
      ],
      "now": "2010-01-02T03:04:10",
    }
    ...
    expected = {
      "items": [
        {
          "dimensions": ["os:Atari", "pool:template"],
          "valid_until_ts": "2010-01-03T07:19:06",
        },
      ],
      "now": "2010-01-02T03:04:10",
    }
...
class BotsApiTest(BaseTest):
  ...
  def test_list_ok(self):
    ...
    bot1 = {
      ...
      "external_ip": "8.8.4.4",
      "first_seen_ts": fmtdate(self.now),
      "is_dead": False,
      "last_seen_ts": fmtdate(self.now),
      "quarantined": False,
      "state": '{"ram":65}',
      "version": "123456789",
    }
    ...
    bot3 = {
      ...
      "external_ip": "8.8.4.4",
      "first_seen_ts": fmtdate(then, DATETIME_NO_MICRO),
      "is_dead": False,
      "last_seen_ts": fmtdate(then, DATETIME_NO_MICRO),
      "quarantined": False,
      "state": '{"ram":65}',
      "version": "123456789",
    }
    ...
    expected = {
      ...
      "death_timeout": unicode(config.settings().bot_death_timeout_secs),
      "now": fmtdate(self.now),
    }
    ...
    expected["items"] = [bot2]
    request = handlers_endpoints.BotsRequest.combined_message_class(
      quarantined=swarming_rpcs.ThreeStateBool.TRUE, dimensions=["pool:default"]
    )
    response = self.call_api("list", body=message_to_dict(request))
    self.assertEqual(expected, response.json)
    ...
    # still work
    request = handlers_endpoints.BotsRequest.combined_message_class(
      quarantined=swarming_rpcs.ThreeStateBool.TRUE, dimensions=["not:existing"]
    )
    response = self.call_api("list", body=message_to_dict(request))
    self.assertEqual(expected, response.json)
  ...
  def test_count_ok(self):
    ...
    expected = {
      ...
      "dead": "0",
      "busy": "4",
      "now": unicode(self.now.strftime(DATETIME_NO_MICRO)),
    }
    request = handlers_endpoints.BotsRequest.combined_message_class()
    response = self.call_api("count", body=message_to_dict(request))
    self.assertEqual(expected, response.json)
...
class BotApiTest(BaseTest):
  ...
  def test_get_deleted_bot(self):
    ...
    expected = {
      ...
      "external_ip": "8.8.4.4",
      "is_dead": False,
      "last_seen_ts": "2010-01-02T03:04:05",
      "quarantined": False,
      "state": '{"foo":0}',
      "version": "123456789",
    }
  ...
  def test_tasks_ok(self):
    ...
    start = (
      utils.datetime_to_timestamp(self.now + datetime.timedelta(seconds=0.5))
      / 1000000.0
    )
    end = (
    ...
    expected = {
      "items": [
        self.gen_run_result(
          ...
          name="philbert",
          performance_stats=self.gen_perf_stats(),
          run_id="5cee870005511",
          started_ts=fmtdate(now_1),
          state="COMPLETED",
          task_id="5cee870005511",
        ),
      ],
      "now": fmtdate(now_1),
    }
    ...
    for sort in (
      swarming_rpcs.TaskSort.COMPLETED_TS,
  ...
  def test_events(self):
    ...
    start = utils.datetime_to_timestamp(first_ticker.first()) / 1000000.0
    end = utils.datetime_to_timestamp(second_ticker.last()) / 1000000.0
    self.set_as_privileged_user()
    ...
    expected = [
      {
        ...
        "event_type": "bot_rebooting",
        "external_ip": unicode(self.source_ip),
        "message": "for the best",
        "quarantined": False,
        "state": state,
        "ts": fmtdate(t5),
        "version": unicode(self.bot_version),
      },
      {
        "authenticated_as": "bot:whitelisted-ip",
        "dimensions": dimensions,
    ...
    start = utils.datetime_to_timestamp(second_ticker.first()) / 1000000.0
    end = utils.datetime_to_timestamp(second_ticker.last()) / 1000000.0
    body = message_to_dict(
      handlers_endpoints.BotEventsRequest.combined_message_class(
        bot_id="bot1", start=start, end=end + 1
  ...
  def test_terminate_admin(self):
    ...
    self.assertEqual({"task_id": "5cee488008810"}, response.json)
