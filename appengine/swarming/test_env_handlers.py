...
class AppTestBase(test_case.TestCase):
  ...
  def setUp(self):
    ...
    self.mock(
      ...
      lambda: "test-swarming.appspot.com",
    )
    ...
    auth.bootstrap_group(
      priv_users_group, [auth.Identity(auth.IDENTITY_USER, "priv@example.com")]
    )
    auth.bootstrap_group(
  ...
  def set_as_swarming_itself(self):
    ...
    self.testbed.setup_env(
      USER_EMAIL=utils.get_service_account_name(), overwrite=True
    )
    auth_testing.reset_local_state()
    auth_testing.mock_get_current_identity(
      self, auth.Identity.from_bytes("user:" + os.environ["USER_EMAIL"])
  ...
  def mock_default_pool_acl(
    ...
    def mocked_fetch_pools_config():
      ...
      return pools_config._PoolsCfg(
        {
          "template": pools_config.init_pool_config(
            name="template",
            rev="pools_cfg_rev",
            ...
            task_template_deployment=pools_config.TaskTemplateDeployment(
              prod=pools_config.TaskTemplate(
                cache=(),
                cipd_package=(
                  pools_config.CipdPackage(".", "some-pkg", "prod-version"),
                ),
              ...
              canary=pools_config.TaskTemplate(
                cache=(),
                cipd_package=(
                  pools_config.CipdPackage(".", "some-pkg", "canary-version"),
                ),
          ...
          "default": pools_config.init_pool_config(
            name="default",
            rev="pools_cfg_rev",
            scheduling_users=frozenset(
              [
    ...
    return auth_api.AuthDB.from_proto(
      ...
      auth_db=replication_pb2.AuthDB(
        ...
        realms={
          ...
          "permissions": [{"name": p.name} for p in _ALL_PERMS],
          "realms": [
            {
              "name": "test:@root",
            },
            {
              ...
              "bindings": [
                {
                  "permissions": [_ALL_PERMS.index(p) for p in permissions],
                  "principals": [auth.get_current_identity().to_bytes()],
                }
              ],
            },
            {
  ...
  def do_handshake(
    ...
    response = self.app.post_json(
      "/swarming/api/v1/bot/handshake", params=params
    ).json
  ...
  def bot_complete_task(self, **kwargs):
    ...
    params = {
      ...
      "output": base64.b64encode("rÉsult string".encode("utf-8")),
  ...
  def bot_run_task(self):
    ...
    self.assertEqual({"must_stop": False, "ok": True}, response)
    return task_id
  ...
  def create_props(**kwargs):
    ...
    out = {
      ...
      "containment": {
        "containment_type": swarming_rpcs.ContainmentType.AUTO,
      },
      "dimensions": [
        {"key": "os", "value": "Amiga"},
  ...
  def client_cancel_task(self, task_id, kill_running):
    return self.endpoint_call(
      ...
      "cancel",
      {"task_id": task_id, "kill_running": kill_running},
    )
  ...
  def client_create_task_cas_input_root(self, properties=None, **kwargs):
    ...
    properties["cas_input_root"] = {
      ...
      "digest": {"hash": "12345", "size_bytes": 1},
    }
  ...
  def gen_perf_stats(**kwargs):
    ...
    out = {
      ...
      "isolated_download": {
        ...
        "items_hot": [30, 40],
        "num_items_cold": "1",
        "total_bytes_items_cold": "20",
        "num_items_hot": "2",
        "total_bytes_items_hot": "70",
      },
      "isolated_upload": {
        "duration": 2.0,
        "items_cold": [1, 2, 40],
        "items_hot": [1, 2, 3, 50],
        "num_items_cold": "3",
        "total_bytes_items_cold": "43",
        "num_items_hot": "4",
        "total_bytes_items_hot": "56",
      },
      "cleanup": {
        "duration": 0.1,
      },
