...
class TaskBackendAPIService(object):
  ...
  def RunTask(self, request, _context):
    ...
    task = task_pb2.Task(
      id=task_pb2.TaskID(id=task_id, target=request.target),
      link="https://%s/task?id=%s&o=true&w=true" % (hostname, task_id),
      update_id=build_task.update_id,
    )
    backend_conversions.convert_task_state_to_status(
      result_summary.state, result_summary.failure, task
    )
  ...
  def CancelTasks(self, request, _context):
    ...
    for tr in ndb.get_multi(request_keys):
      if tr:
        pools.update(
          set(bot_management.get_pools_from_dimensions_flat(tr.tags))
        )
    ...
    query = task_result.filter_query(
    ...
    return backend_pb2.CancelTasksResponse(
      tasks=backend_conversions.convert_results_to_tasks(task_results, task_ids)
    )
  ...
  def ValidateConfigs(self, request, _context):
    ...
    configs = [
      backend_conversions.ingest_backend_config_with_default(config.config_json)
