...
class SwarmingClient(object):
  ...
  def isolate(self, isolate_path):
    ...
    assert p.returncode == 0, (
      "Failed to isolate files. exit_code=%d, cmd=%s" % (p.returncode, cmd)
    )
  ...
  def task_trigger(self, args):
    ...
    assert not self._run_swarming("trigger", cmd), (
      "Failed to trigger a task. cmd=%s" % cmd
    )
  ...
  def task_trigger_with_cas(self, digest, name, extra):
    """Triggers a task and return the task id."""
    h, tmp = tempfile.mkstemp(
      dir=self._tmpdir, prefix="trigger_with_cas", suffix=".json"
    )
    os.close(h)
    cmd = [
      "-user",
  ...
  def task_trigger_raw(self, request):
    ...
    assert ret == 0, "Failed to spawn a task. exit_code=%d, args=%s" % (
      ret,
      args,
    )
  ...
  def task_collect(
    self, task_id, timeout=TIMEOUT_SECS, wait=True, include_output=True
  ):
  ...
  def task_cancel(self, task_id, kill_running=False):
    ...
    assert ret == 0, "Failed to cancel task. exit_code=%d, args=%s" % (
      ret,
      args,
    )
  ...
  def _rotate_logfile(self):
    filename = os.path.join(self._tmpdir, "client_%d.log" % self._index)
    self._index += 1
    return filename
...
def gen_expected(**kwargs):
  expected = {
    "bot_id": unicode(socket.getfqdn().split(".", 1)[0]),
    "output": re.compile("(\\s|\\S)*hi\n"),
    "server_versions": ["N/A"],
    "state": "COMPLETED",
    "tags": [
      "authenticated:bot:whitelisted-ip",
      "pool:default",
      "priority:200",
      "realm:none",
...
def get_bot_dimensions():
  return json.loads(
    subprocess.check_output(
      ["vpython3", os.path.join(APP_DIR, "tools", "os_utilities.py")]
    )
  )["dimensions"]
...
class Test(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.dimensions["bot_config"] = ["bot_config.py"]
    ...
    had_cache = any("caches" == i["key"] for i in old["dimensions"])
    ...
    while True:
      ...
      if not started_ts or new_started_ts != started_ts:
        self.assertNotIn("caches", state["dimensions"])
        break
  ...
  def test_raw_bytes(self):
    ...
    summary = self.gen_expected(
      ...
      output=re.compile("(\\s|\\S)*A﻿�sfs�\\(B"),
      tags=sorted(
        [
          "authenticated:bot:whitelisted-ip",
          "os:" + self.dimensions["os"][0],
          "pool:default",
          "priority:200",
          "realm:none",
          "service_account:none",
  ...
  def test_success_fails(self):
    ...
    tasks = [
      (
        ...
        (self.gen_expected(name="simple_success"), {}),
      ),
      (
        get_cmd("simple_failure", 1),
        (
          self.gen_expected(name="simple_failure", exit_code="1", failure=True),
          {},
        ),
      ),
      (
        get_cmd("ending_simple_success", 0),
        (self.gen_expected(name="ending_simple_success"), {}),
      ),
    ]

    # tuple(task_id, expectation)
    running_tasks = [
      (self.client.task_trigger(args), expected) for args, expected in tasks
    ]
  ...
  def test_with_cas(self):
    ...
    expected_performance_stats = {
      ...
      "isolated_download": {
        ...
        "num_items_cold": unicode(len(items_in)),
        "total_bytes_items_cold": unicode(sum(items_in)),
      },
      "isolated_upload": {
        "items_cold": sorted(items_out),
        "num_items_cold": unicode(len(items_out)),
        "total_bytes_items_cold": unicode(sum(items_out)),
      },
      "cleanup": {},
    }
  ...
  def test_command_env_with_cas(self):
    ...
    content = {
      os.path.join("base", HELLO_WORLD + ".py"): _script("""
        # coding=utf-8
        import os
        import sys
        print(sys.argv[1])
    ...
    expected_summary = self.gen_expected(
      name="command_env",
      output=re.compile("(\\s|\\S)*%s" % re.escape(expected_output)),
      tags=[
        "authenticated:bot:whitelisted-ip",
        "pool:default",
        "priority:200",
  ...
  def test_idempotent_reuse(self):
    content = {HELLO_WORLD + ".py": 'print("hi")\n'}
    ...
    expected_summary = self.gen_expected(
      name="idempotent_reuse2",
      cost_saved_usd=0.02,
      deduped_from=task_id[:-1] + "1",
      tags=[
        "authenticated:bot:whitelisted-ip",
        "pool:default",
        "priority:200",
  ...
  def test_local_cache(self):
    ...
    self.assertNotIn("cache", set(dimensions))
    content = {
      HELLO_WORLD + ".py": _script("""
    ...
    expected_summary["bot_dimensions"] = sorted(
      [{"key": "caches", "value": ["fuu"]}] + expected_summary["bot_dimensions"]
    )
    _, output_root, performance_stats = self._run_with_cas(
      digest,
      "cache_second",
    ...
    expected_performance_stats = {
      ...
      "isolated_download": {
        ...
        # Items are hot.
        "items_hot": sorted(items_in),
        "num_items_hot": unicode(len(items_in)),
        "total_bytes_items_hot": unicode(sum(items_in)),
      },
      "isolated_upload": {
        "items_cold": sorted(items_out),
        "num_items_cold": unicode(len(items_out)),
  ...
  def test_cancel_pending(self):
    ...
    result = self.client.task_result(task_id)
    self.assertEqual("CANCELED", result["state"], result)
  ...
  def test_kill_running(self):
    ...
    self._wait_for_state(task_id, "PENDING", "RUNNING")
    ...
    result = self._wait_for_state(task_id, "RUNNING", "KILLED")
  ...
  def test_no_resource(self):
    ...
    task_id = self.client.task_trigger_raw(request)
    actual_summary, _ = self.client.task_collect(task_id)
    summary = actual_summary["shards"][0]
    # Immediately cancelled.
    self.assertEqual("NO_RESOURCE", summary["state"])
    self.assertTrue(summary["abandoned_ts"])
  ...
  def _make_wait_task(self, name):
    ...
    self._wait_for_state(wait_task_id, "PENDING", "RUNNING")
    yield
    # Double check.
    result = self.client.task_result(wait_task_id)
    self.assertEqual("RUNNING", result["state"], result)
    ...
    self.assertResults(
      self.gen_expected(
        name="wait", tags=tags, output=re.compile("(\\S|\\s)*hi\nhi again\n")
      ),
      actual_summary,
    )

  def _run_with_cas(
    self, digest, name, args, expected_summary, expected_files, deduped
  ):
    ...
    performance_stats = actual_summary["shards"][0].pop(
  ...
  def assertResults(self, expected, result, deduped=False):
    ...
    self.assertTrue(result["shards"][0], result)
    ...
    if result.get("costs_usd") is not None:
      expected.pop("costs_usd", None)
      self.assertLess(0, result.pop("costs_usd"))
    if result.get("cost_saved_usd") is not None:
      expected.pop("cost_saved_usd", None)
      self.assertLess(0, result.pop("cost_saved_usd"))
    self.assertTrue(result.pop("created_ts"))
    self.assertTrue(result.pop("completed_ts"))
    self.assertLess(0, result.pop("duration"))
    ...
    self.assertTrue(result.pop("bot_idle_since_ts"))
    self.assertTrue(result.pop("modified_ts"))
    self.assertTrue(result.pop("started_ts"))

    if getattr(expected.get("output"), "match", None):
    ...
    result["bot_dimensions"] = sorted(
      [d for d in result["bot_dimensions"] if not d["key"] == "python"]
    )
  ...
  def assertPerformanceStatsEmpty(self, actual):
    self.assertLess(0, actual.pop("bot_overhead"))
    self.assertLess(0, actual["cache_trim"].pop("duration"))
    self.assertLessEqual(0, actual["named_caches_install"].pop("duration", 0))
    self.assertLessEqual(0, actual["named_caches_uninstall"].pop("duration", 0))
    self.assertLess(0, actual["cleanup"].pop("duration"))
    self.assertEqual(
      {
        "cache_trim": {},
        "package_installation": {},
  ...
  def assertPerformanceStats(self, expected, actual):
    ...
    self.assertLessEqual(0, actual["named_caches_install"].pop("duration", 0))
    self.assertLessEqual(0, actual["named_caches_uninstall"].pop("duration", 0))
    self.assertLess(0, actual["isolated_download"].pop("duration"))
    self.assertLessEqual(0, actual["isolated_upload"].pop("duration", 0))
    self.assertLess(0, actual["cleanup"].pop("duration"))
...
def main():
  ...
  try:
    ...
    os.mkdir(botdir)
    bot = start_bot.LocalBot(
      servers.swarming_server.url,
      servers.cas_server.address,
      True,
      botdir,
      "vpython3",
    )
    ...
    client = SwarmingClient(
      servers.swarming_server.url, servers.cas_server.address, Test.tmpdir
    )
