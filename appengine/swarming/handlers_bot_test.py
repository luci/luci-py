...
class BotApiTest(test_env_handlers.AppTestBase):
  ...
  def mock_bot_group_config(self, **kwargs):
    ...
    self.mock(
      bot_auth,
      "authenticate_bot",
      lambda *args, **kwargs: (group_cfg, auth_cfg),
    )
  ...
  def test_handshake(self):
    ...
    params = {
      ...
      "state": {"running_time": 0, "sleep_streak": 0},
      "version": "1",
    ...
    self.assertEqual({"dimensions": {}}, response["bot_group_cfg"])
    self.assertEqual(64, len(response["bot_version"]))
    self.assertEqual("v1a", response["server_version"])
  ...
  def test_poll_bad_dimensions(self):
    ...
    expected_event = {
      ...
      "dimensions": {"id": ["bot1"], "pool": ["default"]},
      ...
      "message": "Invalid dimension value. key: foo, value: [u'bar']",
      "owners": [],
      "quarantined": True,
      "session_id": "test-session",
      "state": {
    ...
    self.assertEqual(bot_info.dimensions_flat, ["id:bot1", "pool:default"])
  ...
  def test_poll_session_refresh(self):
    handshake_response = {}
    params = self.do_handshake(
      session_id="fake-session", response_copy=handshake_response
    )
  ...
  def test_poll_session_expiry(self):
    ...
    self.assertEqual(
      ...
      {
        "cmd": "bot_restart",
        "message": "Restarting because the bot session expired",
      },
    )

  def test_poll_with_effective_bot_id(self):
    self.mock_bot_group_config(
      version="default", dimensions={"pool": ["default"]}, is_default=True
    )
    ...
    session = bot_session.unmarshal(response["session"])
    self.assertEqual(
      session.bot_config.rbe_effective_bot_id, "default:dut_id:dut1"
    )
    self.assertEqual(
      session.bot_config.rbe_effective_bot_id_dimension, "dut_id"
    )
  ...
  def test_poll_rbe_in_hybrid_mode(self):
    ...
    params["force"] = False
    response = self.post_json("/swarming/api/v1/bot/poll", params)
    self.assertEqual(response["cmd"], "rbe")
    ...
    _, task_id = self.client_create_task_raw(
      properties={"relative_cwd": "de/ep"}, realm="test:task_realm"
    )
    task_id = task_id[:-1] + "1"
    ...
    params["force"] = True
    response = self.post_json("/swarming/api/v1/bot/poll", params)
    self.assertEqual(response["cmd"], "run")
  ...
  def test_poll_bot_group_config_change(self):
    ...
    self.mock_bot_group_config(
      version="default", dimensions={"pool": ["another-pool"]}, is_default=True
    )
  ...
  def test_poll_task_raw(self):
    ...
    expected = {
      ...
      "manifest": {
        "bot_id": "bot1",
        "bot_authenticated_as": "bot:whitelisted-ip",
        "caches": [],
        "cipd_input": {
          "client_package": {
        ...
        "service_accounts": {
          "system": {"service_account": "none"},
          "task": {"service_account": "none"},
        },
        "task_id": task_id,
        "bot_dimensions": {
          "id": ["bot1"],
  ...
  def test_poll_with_resultdb(self):
    ...
    expected = {
      ...
      "current_invocation": {
        ...
        "update_token": "resultdb-update-token",
      },
    }
  ...
  def test_poll_extra_bot_config(self):
    ...
    params = self.do_handshake()
    self.assertEqual(
      'print("Hi");import sys; sys.exit(1)', params["bot_config"]
    )
  ...
  def test_poll_with_external_scheduler(self):
    ...
    mock.patch(
      "server.external_scheduler.config_for_bot"
    ).start().return_value = es_cfg
    mock.patch(
      "server.external_scheduler.config_for_task"
    ).start().return_value = es_cfg
    # Inject prpc client.
    es_client_mock = mock.Mock()
    mock.patch(
      "server.external_scheduler._get_client"
    ).start().return_value = es_client_mock
    ...
    self.assertEqual(
      ...
      plugin_pb2.AssignTasksRequest(
        scheduler_id="all",
        idle_bots=[
          plugin_pb2.IdleBot(bot_id="bot1", dimensions=bot_dimensions_flat),
        ],
        ...
        notifications=[
          plugin_pb2.NotifyTasksItem(
            task=plugin_pb2.TaskSpec(
              ...
              tags=task["tags"],
              slices=[
                plugin_pb2.SliceSpec(dimensions=task_dimensions_flat),
              ],
              state="PENDING",
    ...
    es_client_mock.AssignTasks.return_value = plugin_pb2.AssignTasksResponse(
      assignments=[
        plugin_pb2.TaskAssignment(
          bot_id="bot1", task_id=task_id, slice_number=0
        ),
      ]
    )
    ...
    es_client_mock.AssignTasks.return_value = plugin_pb2.AssignTasksResponse(
      assignments=[
        plugin_pb2.TaskAssignment(
          bot_id="bot1", task_id=child_task_id, slice_number=0
        ),
      ]
    )
    response = self.post_json("/swarming/api/v1/bot/poll", params)
    ...
    es_client_mock.AssignTasks.return_value = plugin_pb2.AssignTasksResponse(
      assignments=[
        plugin_pb2.TaskAssignment(
          bot_id="bot1", task_id=grand_child_task_id, slice_number=0
        ),
      ]
    )
    response = self.post_json("/swarming/api/v1/bot/poll", params)
  ...
  def test_claim_run(self):
    ...
    self.set_as_bot()
    params = self.do_handshake()
    params["claim_id"] = "some-claim-id"
    params["task_id"] = task_id
    params["task_to_run_shard"] = to_run[0].shard_index
    params["task_to_run_id"] = to_run[0].key.id()
    ...
    self.assertEqual(
      response,
      {
        ...
        "manifest": {
          ...
          "command": ["python", "run_test.py"],
          "containment": {"containment_type": 2},
  ...
  def test_claim_skip(self):
    params = self.do_handshake()
    params["claim_id"] = "some-claim-id"
    params["task_id"] = "5cee400000010"  # doesn't exist
    params["task_to_run_shard"] = 1
    params["task_to_run_id"] = 1
    response = self.post_json("/swarming/api/v1/bot/claim", params)
    self.assertEqual(
      response,
      {
        "cmd": "skip",
        "reason": "No task slice",
        "session": mock.ANY,
      },
    )
  ...
  def test_claim_bad_task_to_run(self):
    ...
    params["task_to_run_id"] = 1
    response = self.app.post_json(
      "/swarming/api/v1/bot/claim", params, expect_errors=True
    )
  ...
  def test_bot_ereporter2_error(self):
    ...
    expected = {
      "id": error_id,
      "url": "http://localhost/restricted/ereporter2/errors/%s" % error_id,
    }
  ...
  def test_bot_event(self):
    ...
    events_to_check = reversed(
      [
        event
        for event in handlers_bot.BotEventHandler.ALLOWED_EVENTS
        if event != "bot_error"
      ]
    )
    expected = [
  ...
  def test_task_complete(self):
    ...
    def _cycle(params, expected, must_stop):
      ...
      self.set_as_bot()
      response = self.post_json("/swarming/api/v1/bot/task_update", params)
      self.assertEqual({"must_stop": must_stop, "ok": True}, response)
      self.set_as_user()
  ...
  def test_task_update_db_failure(self):
    ...
    self.assertEqual({"error": "Failed to update, please retry"}, response)
  ...
  def test_task_update_failure(self):
    ...
    self.assertEqual({"error": "Sorry!"}, response)
  ...
  def test_task_internal_failure(self):
    ...
    expected = [
      {
        "message": "Bot: https://test-swarming.appspot.com/restricted/bot/bot1\n"
        "Task failed: "
  ...
  def test_task_canceled(self):
    ...
    params = self.do_handshake()
    response = self.post_json("/swarming/api/v1/bot/poll", params)
    self.assertEqual("run", response.get("cmd"))
    task_id = response["manifest"]["task_id"]

    # Cancel the task while it's running.
    self.set_as_user()
    response = self.client_cancel_task(task_id, True)
    self.assertEqual({"ok": True, "was_running": True}, response)
    ...
    self.assertEqual({"ok": False, "was_running": False}, response)
  ...
  def test_bot_code_known(self):
    resp = self.app.get(
      "/swarming/api/v1/bot/bot_code/" + self.stable_bot_digest
    )
    self.assertEqual(resp.status_int, 200)
    self.assertEqual(resp.body, "stable-1234+stable-5678")
    resp = self.app.get(
      "/swarming/api/v1/bot/bot_code/" + self.canary_bot_digest
    )
    self.assertEqual(resp.status_int, 200)
  ...
  def test_bot_code_strips_query(self):
    resp = self.app.get(
      "/swarming/api/v1/bot/bot_code/" + self.stable_bot_digest + "?q=123"
    )
    self.assertEqual(resp.status_int, 302)  # Found
    self.assertEqual(
      resp.location,
      "http://localhost/swarming/api/v1/bot/bot_code/" + self.stable_bot_digest,
    )
  ...
  def test_oauth_token_bad_scopes(self):
    ...
    self.assertEqual(
      {"error": '"scopes" must be a list of strings'}, response.json
    )
  ...
  def test_id_token_bad_audience(self):
    ...
    self.assertEqual(400, response.status_code)
    self.assertEqual({"error": '"audience" must be a string'}, response.json)
  ...
  def test_token_task_account(
    ...
    _, task_summary_id = self.client_create_task_raw(
      service_account="blah@example.com", realm="test:task_realm"
    )
    ...
    self.set_as_bot()
    resp = self.bot_poll()
    self.assertEqual("bot1", resp["manifest"]["bot_id"])
    ...
    self.assertEqual(400, response.status_code)
    self.assertEqual(
      {"error": "Wrong task_id: the bot is not executing this task"},
      response.json,
    )
    ...
    self.assertEqual({"error": "Fatal error"}, response.json)
    ...
    self.assertEqual({"error": "Transient error"}, response.json)
  ...
  def test_token_system_account(
    ...
    response = self.app.post_json(
      "/swarming/api/v1/bot/%s" % endpoint,
      params=self.gen_token_request(endpoint, account_id="system"),
    ).json
    self.assertEqual(
      {
        response_key: "blah",
        "expiry": 126240504,
        "service_account": "system@example.com",
      },
      response,
    )
  ...
  def test_token_system_account_none(self, endpoint):
    ...
    self.assertEqual({"service_account": "none"}, response)
