...
def ingest_backend_config(req_backend_config):
  ...
  return json_format.Parse(
    json_config, swarming_pb2.SwarmingTaskBackendConfig()
  )
...
def _compute_task_slices(run_task_req, backend_config, has_secret_bytes):
  ...
  base_slice = task_request.TaskSlice(
    ...
    expiration_secs=int(
      run_task_req.start_deadline.seconds - utils.time_time()
    ),
    properties=task_request.TaskProperties(
      caches=[
        task_request.CacheEntry(
          path=posixpath.join(_CACHE_DIR, cache.path)
          if not cache.path.startswith(_CACHE_DIR)
          else cache.path,
          name=cache.name,
        )
        for cache in run_task_req.caches
...
def convert_backend_task_details(bot_dimensions):
  ...
  if bot_dimensions:
    json_format.ParseDict(
      {"bot_dimensions": bot_dimensions}, task_details_struct
    )
  return task_details_struct


def convert_results_to_fetch_tasks_responses(
  task_results, build_tasks, task_ids
):
  # type: (
  ...
  if len(task_results) != len(build_tasks):
    logging.error(
      "got %d task_results while %d build_tasks",
      len(task_results),
      len(build_tasks),
    )
