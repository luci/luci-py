...
def _validate_length(prop_name, value, maximum):
  if len(value) > maximum:
    raise datastore_errors.BadValueError(
      "too long %s: %d > %d" % (prop_name, len(value), maximum)
    )
...
def _validate_dimensions(_prop, value):
  ...
  if len(value) > maxkeys:
    raise datastore_errors.BadValueError(
      "dimensions can have up to %d keys" % maxkeys
    )
  ...
  for k, values in value.items():
    ...
    if not config.validate_dimension_key(k):
      raise datastore_errors.BadValueError(
        "dimension key must a string that fits %r: %r is invalid"
    ...
    if not values:
      raise datastore_errors.BadValueError(
        "dimensions must be a dict of strings or list of string, not %r" % value
      )
    ...
    for val in values:
      if not val:
        raise datastore_errors.BadValueError(
          "dimensions value must be a string, not None"
        )
    ...
    if len(values) > maxvalues:
      raise datastore_errors.BadValueError(
        "dimension key %r has too many values; maximum is %d" % (k, maxvalues)
      )
    if len(values) > 1:
      logging.warning(
    ...
    if len(values) != len(set(values)):
      raise datastore_errors.BadValueError(
        "dimension key %r has repeated values" % k
      )
    ...
    for val in values:
      ...
      for v in or_values:
        if not config.validate_dimension_value(v):
          raise datastore_errors.BadValueError(
            "dimension key %r has invalid value %r" % (k, val)
          )
    ...
    # Key specific checks.
    if k == "id" and len(values) != 1:
      raise datastore_errors.BadValueError(
        "'id' cannot be specified more than once in dimensions"
      )
    # Do not allow a task to be triggered in multiple pools, as this could
    # cross a security boundary.
    if k == "pool" and (len(values) != 1 or OR_DIM_SEP in values[0]):
      raise datastore_errors.BadValueError(
        "'pool' cannot be specified more than once in dimensions %s" % values
      )
...
def _validate_env_prefixes(prop, value):
  ...
  for k, values in value.items():
    ...
    if not isinstance(values, list) or not all(
      isinstance(v, unicode) for v in values
    ):
      raise TypeError(
...
class FilesRef(ndb.Model):
  ...
  def _pre_put_hook(self):
    ...
    if self.namespace == "sha256-GCP":
      ...
      if (
        not 3 <= len(self.isolatedserver) <= 30
      ) or "://" in self.isolatedserver:
        raise datastore_errors.BadValueError(
...
class CASReference(ndb.Model):
  ...
  def create_from_json(cls, json):
    ...
    if digest:
      h, size = digest.split("/")
    return cls(
      cas_instance=cas_instance, digest=Digest(hash=h, size_bytes=int(size))
    )
...
class TaskProperties(ndb.Model):
  ...
  def is_terminate(self):
    ...
    return (
      ...
      and not self.cipd_input
      and not self.env
      and not self.env_prefixes
      and not self.execution_timeout_secs
      and not self.grace_period_secs
      and not self.io_timeout_secs
      and not self.idempotent
      and not self.outputs
      and not self.has_secret_bytes
      and self.dimensions_data.keys() == ["id"]
    )
  ...
  def _pre_put_hook(self):
    ...
    if "pool" not in self.dimensions_data:
      # Only terminate task may not use 'pool'. Others must specify one.
      raise datastore_errors.BadValueError("'pool' must be used as dimensions")

    if not self.command:
      raise datastore_errors.BadValueError("'command' must be specified")
    ...
    if not self.execution_timeout_secs:
      ...
      raise datastore_errors.BadValueError(
        "'execution_timeout_secs' must be specified"
      )
      ...
      if self.idempotent:
        ...
        if not_pinned:
          raise datastore_errors.BadValueError(
            ...
            "use tags or instance IDs as package versions for %s" % not_pinned
          )
...
class TaskSlice(ndb.Model):
  ...
  def to_dict(self):
    ...
    out = super(TaskSlice, self).to_dict(
      exclude=["properties", "properties_hash"]
    )
...
class TaskRequest(ndb.Model):
  ...
  # information.
  root_task_id = ndb.StringProperty(
    indexed=False, validator=_validate_task_run_id
  )
  ...
  disable_external_scheduler = ndb.BooleanProperty(default=False, indexed=False)
  ...
  def _pre_put_hook(self):
    ...
    if len(self.task_slices) > 1:
      ...
      num_unique = len(
        set(
          utils.encode_to_json(t.properties.to_dict()) for t in self.task_slices
        )
      )
    ...
    # TaskSlice.
    v = self.task_slice(0).properties.dimensions.get("pool")
    for i in range(1, self.num_task_slices):
      t = self.task_slice(i)
      w = t.properties.dimensions.get("pool")
      if v != w:
        raise datastore_errors.BadValueError(
          "each task slice must use the same pool dimensions; %s != %s" % (v, w)
        )
    ...
    if self.pubsub_topic and not pubsub.validate_full_name(
      self.pubsub_topic, "topics"
    ):
      raise datastore_errors.BadValueError(
...
def _get_automatic_tags(request):
  ...
  tags = set(
    (
      ...
      "realm:%s" % (request.realm or "none"),
      "service_account:%s" % (request.service_account or "none"),
      "user:%s" % (request.user or "none"),
      "authenticated:%s" % request.authenticated.to_bytes(),
    )
  )
  if request.parent_task_id:
    tags.add("parent_task_id:%s" % request.parent_task_id)
...
def create_termination_task(bot_id, rbe_instance=None, reason=None):
  ...
  properties = TaskProperties(
    dimensions_data={"id": [unicode(bot_id)]},
  ...
  now = utils.utcnow()
  if reason:
    name = "Terminate %s: %s" % (bot_id, reason)
  else:
    name = "Terminate %s" % bot_id
  request = TaskRequest(
    created_ts=now,
...
def datetime_to_request_base_id(now):
  ...
  return int(round(delta.total_seconds() * 1000.0)) << 20
...
def validate_request_key(request_key):
  ...
  if (
    not root_entity_shard_id
    or len(root_entity_shard_id) != task_pack.DEPRECATED_SHARDING_LEVEL
  ):
    raise ValueError(
...
def _select_task_template(pool, template_apply):
  ...
  canary = deployment.canary and (
    ...
    or (
      template_apply == TEMPLATE_AUTO
      and random.randint(1, 9999) < deployment.canary_chance
    )
  )
  ...
  for envvar in task_template.env:
    ...
    if not envvar.soft:
      ...
      if var_name in (props.env_prefixes or {}):
        raise ValueError(
          "request.env_prefixes[%r] conflicts with pool's template" % var_name
        )
    ...
    if envvar.prefix:
      ...
      props.env_prefixes[var_name] = list(
        envvar.prefix
      ) + props.env_prefixes.get(var_name, [])
...
def validate_priority(priority):
  ...
  if not isinstance(priority, int):
    raise TypeError(
      "priority (%s) must be int type, got %s" % (priority, type(priority))
    )
...
def validate_ping_tolerance(prop_name, ping_tolerance):
  ...
  if (
    ping_tolerance > _MAX_BOT_PING_TOLERANCE_SECS
...
def validate_service_account(prop_name, service_account):
  ...
  if not (
    service_account in ("bot", "none")
    or service_accounts_utils.is_service_account(service_account)
  ):
    raise datastore_errors.BadValueError(
...
def validate_task_run_id(prop_name, run_id):
  ...
  try:
    ...
    raise ValueError("%s (%s) got error: %s" % (prop_name, run_id, e.message))
