...
class PoolsConfigTest(test_case.TestCase):
  ...
  def test_get_pool_config(self):
    ...
    expected1 = pools_config.init_pool_config(
      ...
      scheduling_groups=frozenset(["group2", "group1"]),
      trusted_delegatees={
        auth.Identity(
          "user", "delegatee@example.com"
        ): pools_config.TrustedDelegatee(
          peer_id=auth.Identity("user", "delegatee@example.com"),
          required_delegation_tags=frozenset(["k:tag1", "k:tag2"]),
        ),
      },
      realm="test:pool/realm",
  ...
  def test_rbe_migration_bad_percent_total(self):
    ...
    self.validator_test(
      cfg,
      [
        "pool #0 (abc): rbe_migration: bot_mode_allocation percents should "
        "sum up to 100"
      ],
    )
...
class TestTaskTemplates(TaskTemplateBaseTest):
  ...
  def test_task_template_update_cache(self):
    ...
    self.assertEqual(
      self.tt(cache=[self.PCE(name="hi", path="there")]), tti.finalize(self.ctx)
    )
    ...
    self.assertEqual(
      self.tt(cache=[self.PCE(name="hi", path="nerd")]), tti.finalize(self.ctx)
    )
  ...
  def test_task_template_update_cipd_package(self):
    ...
    tti.update(
      self.ctx,
      self.tt(
        cipd_package=[self.PCP(path="path", pkg="some/pkg", version="oldest")]
      ),
      None,
    )
  ...
  def test_task_template_update_env(self):
    ...
    tti.update(
      self.ctx, self.tt(env=[self.PE(var="VAR", value="1", soft=True)]), None
    )
  ...
  def test_simple_pb(self):
    ...
    self.assertEqual(
      ...
      pools_config.TaskTemplate(
        ...
        cipd_package=(pools_config.CipdPackage("bin", "foo/bar", "latest"),),
        env=(
...
class TestPoolCfgTaskTemplate(TaskTemplateBaseTest):
  ...
  def test_resolve_diamond_inclusion(self):
    ...
    self.assertEqual(
      [x.text for x in self.ctx.result().messages],
      ["template[u'd']: template u'a' included (transitively) multiple times"],
    )
  ...
  def test_resolve_single_deployment(self):
    ...
    self.assertEqual(
      pools_config.TaskTemplateDeployment(
        prod=self.tt(env=[self.PE(var="VAR", value="1")], inclusions="a"),
        canary=self.tt(
          env=(self.PE(var="VAR", value="1"), self.PE(var="WAT", value="yes")),
          inclusions={"a"},
        ),
        canary_chance=5000,
      ),
...
class TestBotMonitoring(TaskTemplateBaseTest):
  ...
  def test_valid_empty(self):
    ...
    self.assertEqual({"hi": ["pool"]}, actual)
  ...
  def test_valid_normal(self):
    ...
    """)
    actual = self.validator_test([bm], [])
    self.assertEqual({"hi": ["a", "pool", "z"]}, actual)
  ...
  def test_bad_repeated_dimension_key(self):
    ...
    dimension_key: "same"
    """)
    self.validator_test([bm], ["bot_monitoring u'hi': duplicate dimension_key"])
