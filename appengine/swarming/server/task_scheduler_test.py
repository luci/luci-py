...
def _gen_properties(**kwargs):
  ...
  args = {
    "command": ["command1"],
    "dimensions": {"os": ["Windows-3.1.1"], "pool": ["default"]},
    "env": {},
...
def _run_result_to_to_run_key(run_result):
  ...
  return task_to_run.request_to_task_to_run_key(
    run_result.request_key.get(), run_result.current_task_slice
  )
...
class TaskSchedulerApiTest(test_env_handlers.AppTestBase):
  def setUp(self):
    ...
    self._enqueue_async_orig = self.mock(
      utils, "enqueue_task_async", self._enqueue_async
    )
  ...
  def _gen_result_summary_reaped(self, **kwargs):
    """Returns the dict for a TaskResultSummary for a pending task."""
    kwargs.setdefault("bot_dimensions", self.bot_dimensions.copy())
    kwargs.setdefault("bot_id", "localhost")
    kwargs.setdefault("bot_idle_since_ts", self.now)
    kwargs.setdefault("bot_version", "abc")
    kwargs.setdefault("state", State.RUNNING)
    kwargs.setdefault("try_number", 1)
  ...
  def _quick_schedule(self, request_id=None, secret_bytes=None, **kwargs):
    ...
    result_summary = task_scheduler.schedule_request(
      request, request_id, secret_bytes=secret_bytes, build_task=build_task
    )
  ...
  def _bot_reap_task(self, bot_dimensions=None, version=None):
    ...
    return task_scheduler.bot_reap_task(
      bot_dimensions, queues, bot_details, _deadline()
    )
  ...
  def _quick_reap(self, **kwargs):
    ...
    if kwargs.get("has_build_task", False):
      self.assertEqual(
        1, len(self._taskqueue_stub.GetTasks("buildbucket-notify"))
      )
      queued_tasks += 1
  ...
  def test_bot_reap_build_task(self):
    ...
    self._assert_buildbucket_update_status(
      pub_sub_calls[0][1]["message"], common_pb2.STARTED
    )
  ...
  @parameterized.expand(
    [
      (
        {
          "pool": ["default"],
          "os": ["Windows-3.1.1|Windows-3.2.1"],
        },
      ),
      (
        {
  ...
  def test_bot_reap_tasks_using_or_dimensions(self):
    ...
    bot1_dimensions["id"] = ["bot1"]
    bot1_dimensions["os"] = ["v1", "v2"]
    bot1_dimensions["gpu"] = ["nv", "sega"]
    ...
    self.assertEqual("bot1", run_result.bot_id)
    to_run_key = _run_result_to_to_run_key(run_result)
    self.assertIsNone(to_run_key.get().queue_number)
    self.assertEqual(
      State.COMPLETED,
      _bot_update_task(
        run_result.key, bot_id="bot1", exit_code=0, duration=0.1
      ),
    )
    ...
    bot2_dimensions["id"] = ["bot2"]
    bot2_dimensions["os"] = ["v2"]
    bot2_dimensions["gpu"] = ["amd", "sega"]
    self._register_bot(bot2_dimensions)

    _, _, run_result = self._bot_reap_task()
    self.assertEqual("bot2", run_result.bot_id)
    to_run_key = _run_result_to_to_run_key(run_result)
    self.assertIsNone(to_run_key.get().queue_number)
    self.assertEqual(
      State.COMPLETED,
      _bot_update_task(
        run_result.key, bot_id="bot2", exit_code=0, duration=0.1
      ),
    )
    self.assertEqual(1, self.execute_tasks())
  ...
  def test_schedule_request_no_capacity_backend_task(self):
    ...
    self.assertEqual(pub_sub_calls[0][1]["topic"], "backend_pubsub_topic")
    self._assert_buildbucket_update_status(
      pub_sub_calls[0][1]["message"], common_pb2.INFRA_FAILURE
    )
  ...
  def test_schedule_request_resultdb(self):
    ...
    with mock.patch(
      ...
      mock.Mock(side_effect=self._mock_create_invocation_async),
    ) as mock_call:
    ...
    self.assertEqual(
      result_summary.request_key.get().resultdb_update_token,
  ...
  def test_schedule_request_rbe_mode(self):
    ...
    self.assertEqual(
      "sample-app-1d69b9f088008910-0", enqueued[0][1].key.get().rbe_reservation
    )
  ...
  def test_bot_reap_task_expired(self):
    ...
    self.assertEqual(
      ...
      ts_mon_metrics._task_state_change_schedule_latencies.get(
        fields=_update_fields_schedule(status=State.to_string(State.EXPIRED))
      ).sum,
    )
  ...
  def test_resultdb_task_expired(self):
    ...
    with mock.patch(
      ...
      mock_call.assert_called_once_with(
        "1d69b9f088008911", "infra:try", mock.ANY
      )
    ...
    with mock.patch(
      ...
      to_run_key = task_to_run.request_to_task_to_run_key(request, 0)
      self.mock_now(self.now, 60)
      task_scheduler._expire_slice(
        request, to_run_key, State.EXPIRED, None, False, 1, True, "some-reason"
      )
      mock_call.assert_called_once_with(
        "1d69b9f088008911", "resultdb-update-token"
      )

    self.execute_tasks()

  def _setup_es(self, allow_es_fallback, disable_percent=None):
    ...
    external_schedulers = [
      pools_config.ExternalSchedulerConfig(
        address=es_address,
        id=es_id,
        dimensions=set(["foo:bar", "label-pool:CTS"]),
        all_dimensions=None,
        any_dimensions=None,
        enabled=False,
  ...
  def test_bot_reap_task_es_with_pending_task_es_disabled(self):
    self._setup_es(False, disable_percent=100)
    notify_calls = self._mock_es_notify()
    ...
    # The task was not sent to external scheduler.
    self.assertEqual(len(notify_calls), 0)
    self._mock_es_assign(None, 0)

    # Able to successfully reap given PENDING task from native scheduler.
    request, _, _ = self._bot_reap_task()
    self.assertEqual(request.task_id, result_summary.task_id)
    self.assertTrue(request.disable_external_scheduler)

  def test_bot_reap_task_for_nonexternal_pool(self):
    self._setup_es(False)
    notify_calls = self._mock_es_notify()
    dimensions = {"os": ["Windows-3.1.1"], "pool": ["label-pool:CTS"]}
    slices = [
      task_request.TaskSlice(
        expiration_secs=60,
  ...
  def test_schedule_request_slice_use_first_after_expiration(self):
    ...
    self._quick_schedule(
      task_slices=[
        task_request.TaskSlice(
          expiration_secs=180, properties=_gen_properties(io_timeout_secs=61)
        ),
        task_request.TaskSlice(
    ...
    self._quick_schedule(
      task_slices=[
        ...
        task_request.TaskSlice(
          expiration_secs=180,
          properties=_gen_properties(
            dimensions={"pool": ["default"], "os": ["Atari"]}
          ),
        ),
      ]
    )
  ...
  def _task_deduped(
    self, new_ts, deduped_from, task_id, now=None, created_ts=None
  ):
  ...
  def test_task_idempotent(self):
    ...
    new_ts = self.mock_now(
      self.now, config.settings().reusable_task_age_secs - 1
    )
    self._task_deduped(
      new_ts,
      task_id,
    ...
    self.assertEqual(
      1000.0,
      ts_mon_metrics._task_state_change_schedule_latencies.get(
        fields=_update_fields_schedule(status=State.to_string(State.COMPLETED))
      ).sum,
    )
  ...
  def test_task_idempotent_result_with_old_hash(self):
    ...
    result.properties_hash = first_request.task_slices[
      0
    ].calculate_properties_hash(None)
    result.put()
    # Second task is deduped against first task.
    new_ts = self.mock_now(
      self.now, config.settings().reusable_task_age_secs - 1
        ...
        task_request.TaskSlice(
          ...
          properties=_gen_properties(
            ...
            cas_input_root=task_request.CASReference(
              ...
              digest=task_request.Digest(hash="1" * 32, size_bytes=1),
            ),
          ),
          wait_for_capacity=False,
        ),
  ...
  def test_task_timeout(self):
    ...
    self.assertEqual(
      State.TIMED_OUT, _bot_update_task(run_result.key, hard_timeout=True)
    )
    self.assertEqual(1, self.execute_tasks())
    run_result = run_result.key.get()
  ...
  def test_get_results(self):
    ...
    self.assertEqual(
      ...
      ts_mon_metrics._task_state_change_schedule_latencies.get(
        fields=_update_fields_schedule(status=State.to_string(State.RUNNING))
      ).sum,
    )
    ...
    with self.assertRaises(datastore_errors.BadValueError):
      self._quick_schedule(
        task_slices=[
          task_request.TaskSlice(
            expiration_secs=60,
            properties=_gen_properties(dimensions={"id": ["abc"]}),
            wait_for_capacity=False,
          ),
        ]
      )
  ...
  def test_bot_update_pubsub_error(self):
    ...
    self.assertLessEqual(
      0,
      ts_mon_metrics._task_state_change_pubsub_notify_latencies.get(
        fields=_update_fields_pubsub(http_status_code=404)
      ).sum,
    )
  ...
  def test_bot_update_buildbucket_pubsub_ok(self):
    ...
    self._assert_buildbucket_update_status(
      pub_sub_calls[1][1]["message"], common_pb2.SUCCESS
    )
  ...
  def test_bot_update_buildbucket_pubsub_ok_failed_task(self):
    ...
    self._assert_buildbucket_update_status(
      pub_sub_calls[1][1]["message"], common_pb2.FAILURE
    )
    self.assertEqual(pub_sub_calls[1][1]["topic"], "backend_pubsub_topic")
    ...
    task_scheduler.task_buildbucket_update(
      {
        ...
        "state": task_result.State.CLIENT_ERROR,
        "update_id": 12345678000000001,
      }
    )
    self.assertEqual(2, len(pub_sub_calls))
  ...
  def test_bot_update_child_with_cancelled_parent(self):
    ...
    ok, was_running = task_scheduler.cancel_task(
      ...
      parent_run_result.result_summary_key,
      True,
      None,
    )
    ...
    self.assertEqual(
      State.KILLED,
      _bot_update_task(
        child_run_result.key, bot_id="localhost2", exit_code=0, duration=0.1
      ),
    )
    ...
    # completed.
    self.assertEqual(
      State.KILLED, _bot_update_task(child_run_result2.key, bot_id="localhost3")
    )
  ...
  def test_bot_terminate_task(self):
    ...
    self.assertEqual(
      None,
      task_scheduler.bot_terminate_task(
        run_result.key,
        "localhost",
        self.now,
        {"missing_cas": None, "missing_cipd": []},
      ),
    )
    expected = self._gen_result_summary_reaped(
  ...
  def test_bot_terminate_backend_task(self):
    ...
    self._assert_buildbucket_update_status(
      pub_sub_calls[2][1]["message"], common_pb2.INFRA_FAILURE
    )
    status = State.to_string(State.BOT_DIED)
    self.assertLessEqual(
      0,
  ...
  def test_bot_terminate_canceled_task(self):
    ...
    err = task_scheduler.bot_terminate_task(
      run_result.key,
      "localhost",
      start_time,
      {"missing_cas": None, "missing_cipd": []},
    ...
    self.assertLessEqual(
      0,
      ts_mon_metrics._dead_task_detection_latencies.get(
        fields={
          "pool": "default",
          "cron": False,
        }
  ...
  def test_bot_terminate_task_missing_cas(self):
    ...
    self.assertEqual(
      None,
      task_scheduler.bot_terminate_task(
        run_result.key, "localhost", self.now, client_error
      ),
    )
  ...
  def test_bot_terminate_task_wrong_bot(self):
    run_result = self._quick_reap()
    expected = (
      "Bot bot1 sent task kill for task 1d69b9f088008911 owned by bot localhost"
    )
    err = task_scheduler.bot_terminate_task(
      run_result.key, "bot1", 0, {"missing_cas": None, "missing_cipd": []}
    )
  ...
  def test_cancel_task_backend_task(self):
    ...
    self._assert_buildbucket_update_status(
      pub_sub_calls[0][1]["message"], common_pb2.CANCELED
    )
  ...
  def test_cancel_task_bot_id(self):
    ...
    ok, was_running = task_scheduler.cancel_task(
      run_result.request_key.get(), run_result.result_summary_key, True, "foo"
    )
    self.assertEqual(False, ok)
    self.assertEqual(True, was_running)
    self.assertEqual(1, self.execute_tasks())
  ...
  def test_cancel_tasks(self):
    ...
    query = task_result.get_result_summaries_query(
      start=None, end=None, sort="created_ts", state="pending_running", tags=[]
    )
  ...
  def test_cancel_tasks_skip_running(self):
    ...
    query = task_result.get_result_summaries_query(
      start=None, end=None, sort="created_ts", state="pending", tags=[]
    )
    ...
    self.assertEqual(
      1000.0,
      ts_mon_metrics._task_state_change_schedule_latencies.get(
        fields=_update_fields_schedule(status=State.to_string(State.CANCELED))
      ).sum,
    )
  ...
  def test_cancel_tasks_conditions(self):
    ...
    pending_result_1 = self._quick_schedule(
      task_slices=task_slices, manual_tags=["tag:1", "tag:2"]
    )

    pending_result_2 = self._quick_schedule(
      task_slices=task_slices, manual_tags=["tag:2", "tag:1"]
    )
    self._quick_schedule(
      task_slices=task_slices, manual_tags=["tag:3", "tag:2"]
    )
    ...
    self.assertItemsEqual(
      [res.task_id for res in results], [res.task_id for res in pending_results]
    )
    self.execute_tasks()
  ...
  def test_route_notification_to_Go(self):
    ...
    utils.enqueue_task.assert_has_calls(
      [
        mock.call(
          ("/internal/tasks/t/pubsub-go/1d69b9f088008910"),
          "pubsub-v2",
          transactional=True,
          payload=mock.ANY,
        ),
        mock.call(
          "/internal/tasks/t/buildbucket-notify-go/1d69b9f088008910",
  ...
  def test_expire_slice(self):
    ...
    self.assertEqual(ttr.dimensions, {"pool": ["default"], "prop": ["a"]})
    del enqueued[:]

    # Expire this pending slice.
    task_scheduler.expire_slice(
      ttr.key, task_result.State.NO_RESOURCE, None, "some-reason"
    )
    ...
    self.assertEqual(ttr.dimensions, {"pool": ["default"], "prop": ["b"]})
    del enqueued[:]
    ...
    result_summary = self._quick_schedule(
      task_slices=[
        task_request.TaskSlice(
          expiration_secs=600,
          properties=_gen_properties(
            dimensions={"pool": ["default"], "item": ["1"]}
          ),
        ),
        task_request.TaskSlice(
          expiration_secs=600,
          properties=_gen_properties(
            dimensions={"pool": ["default"], "item": ["2"]}
          ),
        ),
        task_request.TaskSlice(
          expiration_secs=600,
          properties=_gen_properties(
            dimensions={"pool": ["default"], "item": ["3"]}
          ),
        ),
        task_request.TaskSlice(
          expiration_secs=600,
          properties=_gen_properties(
            dimensions={"pool": ["default"], "item": ["4"]}
          ),
        ),
      ],
      scheduling_algorithm=pools_pb2.Pool.SCHEDULING_ALGORITHM_FIFO,
  ...
  def test_cron_handle_bot_died(self):
    ...
    now_0 = self.now
    now_1 = self.mock_now(
      self.now + datetime.timedelta(seconds=request.bot_ping_tolerance_secs), 1
    )
    self.assertEqual(
      ([run_result.task_id], 0), task_scheduler.cron_handle_bot_died()
    )
    self.assertEqual(1, self.execute_tasks())
  ...
  def test_bot_poll_http_500_but_bot_reapears_after_BOT_PING_TOLERANCE(self):
    ...
    self.assertEqual(
      ([to_run_key_1.get().task_id], 0), task_scheduler.cron_handle_bot_died()
    )
    ...
    self.assertEqual(
      None,
      _bot_update_task(
        run_result.key, bot_id="localhost-second", exit_code=0, duration=0.1
      ),
    )
  ...
  def test_cron_handle_bot_died_same_bot_denied(self):
    ...
    now_2 = self.mock_now(
      self.now + datetime.timedelta(seconds=request.bot_ping_tolerance_secs), 2
    )
    request, _, run_result = self._bot_reap_task()
  ...
  def test_cron_handle_bot_died_ignored_expired(self):
    ...
    self.assertEqual(
      (["1d69b9f088008911"], 0), task_scheduler.cron_handle_bot_died()
    )
  ...
  def test_cron_handle_external_cancellations(self):
    ...
    external_schedulers = [
      pools_config.ExternalSchedulerConfig(
        es_address, es_id, None, None, None, True, True, None
      ),
      pools_config.ExternalSchedulerConfig(
        es_address, es_id, None, None, None, True, True, None
      ),
      pools_config.ExternalSchedulerConfig(
        es_address, es_id, None, None, None, False, True, None
      ),
    ]
  ...
  def test_cron_handle_external_cancellations_none(self):
    ...
    external_schedulers = [
      pools_config.ExternalSchedulerConfig(
        es_address, es_id, None, None, None, True, True, 0
      ),
      pools_config.ExternalSchedulerConfig(
        es_address, es_id, None, None, None, True, True, 0
      ),
      pools_config.ExternalSchedulerConfig(
        es_address, es_id, None, None, None, False, True, 0
      ),
    ]
    self.mock_pool_config("es-pool", external_schedulers=external_schedulers)
    known_pools = pools_config.known()
      ...
      if pool == name:
        return pools_config.init_pool_config(
          ...
          scheduling_algorithm=(
            scheduling_algorithm or pools_pb2.Pool.SCHEDULING_ALGORITHM_FIFO
          ),
        )
      return None
  ...
  def mock_delegation(self, peer_id, tags):
    ...
    self.mock(
      auth, "get_delegation_token", lambda: delegation_pb2.Subtoken(tags=tags)
    )
    ...
    r = self._quick_schedule(
      task_slices=[
        task_request.TaskSlice(
          expiration_secs=60,
          properties=_gen_properties(
            dimensions={"pool": ["default"], "foo": ["bar"]}
          ),
          wait_for_capacity=True,
        ),
        task_request.TaskSlice(
          expiration_secs=60,
          properties=_gen_properties(
            dimensions={"pool": ["default"], "foo": ["qux"]}
          ),
          wait_for_capacity=True,
        ),
      ],
  ...
  def test_task_expire_with_invalid_slice_index(self):
    ...
    ttr, _ = task_scheduler._ensure_active_slice(
      result_summary.request, invalid_slice_index
    )
    self.assertEqual(State.PENDING, result_summary.state)
    self.assertEqual(0, result_summary.current_task_slice)
