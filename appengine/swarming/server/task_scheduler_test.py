...
class TaskSchedulerApiTest(test_case.TestCase):
  ...
  def test_get_results(self):
    ...
    self.assertEqual(
        (True, False),
        task_scheduler.bot_update_task(
        run_result.key, 'localhost', 1, 'Bar22', 0, 0, 0.2, False, False, 0.1))
    result_summary, run_results = get_results(request.key)
    expected = {
      'abandoned_ts': None,
      'bot_id': u'localhost',
  ...
  def test_exit_code_failure(self):
    ...
    self.assertEqual(
        (True, True),
        task_scheduler.bot_update_task(
        run_result.key, 'localhost', 0, 'Foo1', 0, 0, 0.1, False, False, 0.1))
    self.assertEqual(
        (True, False),
        task_scheduler.bot_update_task(
        run_result.key, 'localhost', 1, 'Bar22', 0, 1, 0.2, False, False, 0.1))
    result_summary, run_results = get_results(request.key)
  ...
  def test_bot_update_task(self):
    ...
    self.assertEqual(
        (True, True),
        task_scheduler.bot_update_task(
            run_result.key, 'localhost', 0, 'hi', 0, 0, 0.1, False, False, 0.1))
    self.assertEqual(
        (True, False),
        task_scheduler.bot_update_task(
  ...
  def _bot_update_timeouts(self, hard, io):
    ...
    self.assertEqual(
        (True, True),
        task_scheduler.bot_update_task(
            run_result.key, 'localhost', 0, 'hi', 0, 0, 0.1, hard, io, 0.1))
    expected = {
      'abandoned_ts': None,
      'bot_id': u'localhost',
      'bot_version': u'abc',
  ...
  def test_cron_handle_bot_died(self):
    ...
    self.assertEqual(
        (True, True),
        task_scheduler.bot_update_task(
            run_result.key, 'localhost-second', 0, 'Foo1', 0, 0, 0.1, False,
            False, 0.1))
    expected = {
      'abandoned_ts': None,
