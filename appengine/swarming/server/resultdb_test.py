...
class ResultDBTest(test_case.TestCase):
  ...
  @ndb.tasklet
  def _mock_call_resultdb_recorder_api_async(
    self, _method, _request, response_headers, **_kwargs
  ):
  ...
  def test_create_invocation_async(self):
    ...
    with mock.patch(
      ...
      update_token = resultdb.create_invocation_async(
        "task001", "infra:try", datetime.datetime(2020, 1, 2, 3, 4, 5)
      ).get_result()
      ...
      mock_call.assert_called_once_with(
        ...
        {
          "invocation": {
            "producerResource": "//test-swarming.appspot.com/tasks/task001",
            "realm": "infra:try",
            "deadline": "2020-01-02T03:04:05Z",
          },
  ...
  def test_create_invocation_async_no_update_token(self):
    with mock.patch(
      ...
      with self.assertRaisesRegexp(
        AssertionError, "^response_headers should have valid update-token: {}$"
      ):
  ...
  def test_call_resultdb_recorder_api(self):
    ...
    with (
      mock.patch("server.config.settings") as mock_settings,
      mock.patch(
        "components.net.json_request_async"
      ) as mock_json_request_async,
    ):
      ...
      resultdb._call_resultdb_recorder_api_async(
        "FinalizeInvocation", {}
      ).get_result()
