...
class TestBase(test_case.TestCase):
  ...
  def mock_json_request(
    self, expected_url, expected_payload, expected_project_id, response
  ):
    calls = []

    def mocked(
      url=None, method=None, payload=None, project_id=None, scopes=None
    ):
      calls.append(url)
...
class TaskAccountTokenTest(TestBase):
  ...
  def test_access_token_with_realm(self):
    ...
    self.mock_json_request(
      ...
      expected_payload={
        "tokenKind": service_accounts.TOKEN_KIND_ACCESS_TOKEN,
  ...
  def test_id_token_with_realm(self):
    ...
    self.mock_json_request(
      ...
      "tokenserver.minter.TokenMinter/MintServiceAccountToken",
      expected_payload={
        "tokenKind": service_accounts.TOKEN_KIND_ID_TOKEN,
        "serviceAccount": "service-account@example.com",
        "realm": "test:realm",
        "oauthScope": None,
...
class SystemAccountTokenTest(test_case.TestCase):
  ...
  def test_none(self):
    self.assertEqual(
      ...
      service_accounts.get_system_account_token(
        None, service_accounts.TOKEN_KIND_ACCESS_TOKEN, scopes=["scope"]
      ),
    )
  ...
  def test_bot(self):
    self.assertEqual(
      ("bot", None),
      service_accounts.get_system_account_token(
        "bot", service_accounts.TOKEN_KIND_ACCESS_TOKEN, scopes=["scope"]
      ),
    )
