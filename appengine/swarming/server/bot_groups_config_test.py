...
TEST_CONFIG = bots_pb2.BotsCfg(
  ...
  bot_group=[
    ...
    bots_pb2.BotGroup(
      auth=[bots_pb2.BotAuth(ip_whitelist="bots")], dimensions=["pool:default"]
    ),
  ],
)
...
EXPECTED_GROUP_2 = bot_groups_config._make_bot_group_config(
  ...
  bot_config_script_sha256="566238a1eb9839809ff20c120387d91042c3efce7d7f30d16470caec93740e1b",
  system_service_account="bot",
  logs_cloud_project="chrome-infra-logs",
  is_default=False,
)
...
class BotGroupsConfigTest(test_case.TestCase):
  ...
  def test_get_bot_group_config(self):
    ...
    self.assertEquals(
      EXPECTED_GROUP_1, bot_groups_config.get_bot_group_config("bot1--zzz")
    )
    self.assertEquals(
      EXPECTED_GROUP_2, bot_groups_config.get_bot_group_config("bot1--xxx")
    )
    self.assertEquals(
  ...
  def test_trusted_dimensions_invalid(self):
    ...
    self.validator_test(
      cfg, ["trusted_dimensions: invalid dimension key u'pool:blah'"]
    )
  ...
  def test_empty_prefix(self):
    cfg = bots_pb2.BotsCfg(
      bot_group=[bots_pb2.BotGroup(bot_id_prefix=[""], auth=DEFAULT_AUTH_CFG)]
    )
    self.validator_test(
  ...
  def test_duplicate_bot_and_prefix_ids(self):
    ...
    self.validator_test(
      ...
      [
        (
          'bot_group #1: bot_id "xyz" was already mentioned as bot_id_prefix '
          "in group #0"
        ),
        (
          'bot_group #1: bot_id_prefix "abc" is already specified as bot_id '
          "in group #0"
        ),
        (
          'bot_group #2: bot_id_prefix "foo" is already specified as bot_id '
          "in group #2"
        ),
      ],
    )
  ...
  def test_bad_dimension_not_kv(self):
    ...
    self.validator_test(cfg, ["bot_group #0: bad dimension u'not_kv_pair'"])
  ...
  def test_bad_dimension_bad_dim_key(self):
    ...
    self.validator_test(
      cfg,
      [
        "bot_group #0: bad dimension u'blah####key:value:value'",
      ],
    )
  ...
  def test_intersecting_prefixes(self):
    ...
    self.validator_test(
      ...
      [
        (
          'bot_group #1: bot_id_prefix "abc-def-" contains prefix "abc-", '
          "defined in group #0, making group assigned for bots with prefix "
          '"abc-" ambigious'
        ),
        (
          'bot_group #3: bot_id_prefix "xyz-" is subprefix of "xyz-def-", '
  ...
  def test_two_default_groups(self):
    ...
    self.validator_test(
      cfg, ["bot_group #1: group #0 is already set as default"]
    )
...
class CacheTest(test_case.TestCase):
  ...
  def test_expands_bot_config_scripts_ok(self):
    ...
    self.assertEqual({"bots.cfg": 1, "scripts/script.py": 1}, calls)
