...
def _bot_event(
  ...
  **kwargs,
  ...
  if not authenticated_as:
    authenticated_as = "bot:%s.domain" % bot_id
...
def _ensure_bot_info(bot_id="id1", **kwargs):
...
def _gen_bot_info(**kwargs):
  ...
  out = {
    ...
    "expire_at": now + bot_management._OLD_BOT_INFO_CUT_OFF,
    "external_ip": "8.8.4.4",
    "first_seen_ts": now,
    "id": "id1",
    "idle_since_ts": None,
...
class BotManagementTest(test_case.TestCase):
  ...
  def test_BotEvent_proto_busy(self):
    ...
    self.assertEqual("task1", actual.bot.current_task_id)
  ...
  def test_BotEvent_proto_events(self):
    ...
    for name in bot_management.BotEvent.ALLOWED_EVENTS:
      event_key = _bot_event(
        event_type=name, bot_id="id1", dimensions=dimensions
      )
      if name in ("task_update", "bot_polling"):
  ...
  def test_BotEvent_proto_maintenance(self):
    ...
    expected = swarming_pb2.BotEvent(
      ...
      bot=swarming_pb2.Bot(
        bot_id="id1",
        dimensions=[
          swarming_pb2.StringListPair(key="id", values=["id1"]),
        ],
        ...
        info=swarming_pb2.BotInfo(
          supplemental=struct_pb2.Struct(
            fields={
              "ram": struct_pb2.Value(number_value=65),
            }
          ),
          version=_VERSION,
          external_ip="8.8.4.4",
  ...
  def test_BotEvent_proto_quarantine(self):
    ...
    event_key = _bot_event(
      event_type="bot_connected",
      bot_id="id1",
      dimensions={"id": ["id1"], "pool": ["next", "previous"]},
      state={"ram": 65, "quarantined": "sad bot"},
      quarantined=True,
    )
    actual = swarming_pb2.BotEvent()
    event_key.get().to_proto(actual)
    expected = swarming_pb2.BotEvent(
      ...
      bot=swarming_pb2.Bot(
        ...
        dimensions=[
          swarming_pb2.StringListPair(key="id", values=["id1"]),
          swarming_pb2.StringListPair(key="pool", values=["next", "previous"]),
        ],
        ...
        info=swarming_pb2.BotInfo(
          supplemental=struct_pb2.Struct(
            fields={
              "quarantined": struct_pb2.Value(string_value="sad bot"),
              "ram": struct_pb2.Value(number_value=65),
            }
          ),
          version=_VERSION,
  ...
  def test_bot_event_reset_task(
    self, start_event, event, reset_task, skip_store_event
  ):
    bot_id = "id1"
    task_id = "12311"
    task_name = "yo"
  ...
  def test_get_events_query(self):
    ...
    expected = [_gen_bot_event(event_type="bot_connected")]
    self.assertEqual(
  ...
  def test_get_latest_info_race_condition_crbug_1407381(self):
    ...
    with mock.patch(
      "server.bot_management.get_events_query",
      side_effect=get_events_query_mock,
    ) as m:
      fetched, alive = bot_management.get_latest_info("bot-id")
      self.assertEqual(stored.dimensions_flat, fetched.dimensions_flat)
  ...
  def test_bot_event_poll_sleep(self, event_type):
    ...
    expected_event = _gen_bot_event(
      event_type=event_type, idle_since_ts=utils.utcnow()
    )
  ...
  def test_bot_event_busy(self):
    ...
    expected = [
      _gen_bot_event(event_type="request_task", task_id="12311", ts=t2),
      _gen_bot_event(event_type="bot_connected", ts=t1),
    ]
    self.assertEqual(
  ...
  def test_bot_event_update_dimensions(self):
    ...
    _bot_event(
      bot_id=bot_id, event_type="bot_connected", dimensions=dimensions_invalid
    )
    self.assertEqual(
      bot_info_key.get().dimensions_flat, ["id:id1", "pool:default"]
    )
    ...
    self.assertEqual(
      ...
      ["id:id1", "os:Android", "pool:default"],
    )
  ...
  def test_get_root_key(self):
    self.assertEqual(
      ndb.Key(bot_management.BotRoot, "foo"), bot_management.get_root_key("foo")
    )
