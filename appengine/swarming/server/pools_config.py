...
CipdServer = collections.namedtuple(
  "CipdServer",
  [
    "server",
    "package_name",
...
class TaskTemplate(_TaskTemplate):
  ...
  class _Intermediate(object):
    ...
    def finalize(self, ctx):
      ...
      return TaskTemplate(
        cache=tuple(
          CacheEntry(name, path) for name, path in sorted(self.cache.items())
        ),
        cipd_package=tuple(
...
def _resolve_external_schedulers(external_schedulers):
  ...
  return tuple(
    ExternalSchedulerConfig(
      e.address,
      e.id,
      frozenset(e.dimensions),
      frozenset(e.all_dimensions),
      frozenset(e.any_dimensions),
      e.enabled,
      e.allow_es_fallback,
      e.disable_percent,
    )
...
def _vaildate_rbe_migration(ctx, msg):
  ...
  total = (
    allocs.get(BotMode.SWARMING, 0)
    + allocs.get(BotMode.HYBRID, 0)
  ...
  for msg in cfg.pool:
    for name in msg.name:
      pools[name] = init_pool_config(
        ...
        default_task_realm=(
          msg.default_task_realm if msg.default_task_realm else None
        ),
        ...
        scheduling_algorithm=(
          msg.scheduling_algorithm
          or pools_pb2.Pool.SCHEDULING_ALGORITHM_UNKNOWN
        ),
        informational_dimension_re=(
          msg.informational_dimension_re
          if msg.informational_dimension_re
          else None
        ),
      )
