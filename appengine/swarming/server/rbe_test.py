...
class RBETest(test_case.TestCase):
  ...
  def pool_config_func(
    ...
    return pool_config_func(
      rbe_instance, rbe_mode_percent, allocs, effective_bot_id_dimension
    )
  ...
  def test_get_rbe_config_for_bot(self, quasi_random_100, pool_config):
    ...
    pools["many-modes"] = pool_config_func(
      "instance",
      allocs={
        "SWARMING": 10,
        "HYBRID": 60,
    ...
    pools["pool-c"] = pool_config_func(
      "instance-c",
      allocs={
        "RBE": 100,
      },
    )
    pools["pool-d"] = pool_config_func(
      "instance-d",
      allocs={
        "RBE": 100,
      },
  ...
  def test_quasi_random_100(self):
    for i in range(1000):
      val = rbe._quasi_random_100("bot-%d" % i)
...
class EnqueueTest(test_case.TestCase):
  ...
  def test_enqueue_rbe_task(
    ...
    self.assertEqual(
      args, ("/internal/tasks/t/rbe-enqueue/2ed6c6804c8002a10-2", "rbe-enqueue")
    )
    self.assertEqual(
      kwargs,
      {
        "payload": {
          "body": {
            "payload": {
              "reservationId": "sample-app-2ed6c6804c8002a10-2",
              "taskId": "2ed6c6804c8002a10",
              "sliceIndex": 2,
              "taskToRunId": "33",
              "taskToRunShard": 11,
              "debugInfo": {
                "created": "2112-01-01T01:01:01Z",
                "pySwarmingVersion": "v1a",
                "taskName": "some-name",
              },
            },
            "rbeInstance": "some-instance",
            "executionTimeout": "1030s",
            "expiry": "2112-01-01T01:07:10Z",
            "requestedBotId": "bot-id",
            "constraints": [
              {"key": "dim1", "allowedValues": ["val1"]},
              {"key": "dim1", "allowedValues": ["val2", "val3"]},
              {"key": "dim2", "allowedValues": ["val4"]},
              {"key": "name", "allowedValues": ["2"]},
              {"key": "pool", "allowedValues": ["pool"]},
            ],
            "priority": 123,
            "schedulingAlgorithm": "SCHEDULING_ALGORITHM_LIFO",
  ...
  @mock.patch("components.utils.utcnow")
  @mock.patch("random.getrandbits")
  def test_enqueue_rbe_task_with_effective_bot_id(
    self, getrandbits, utcnow, enqueue_task, pool_config
  ):
    getrandbits.return_value = 42
    utcnow.return_value = datetime.datetime(2112, 1, 1, 1, 1, 1)
    ...
    pool_config.side_effect = lambda pool: pools[pool]
    pools["pool"] = pool_config_func(
      "instance", allocs={"RBE": 100}, effective_bot_id_dimension="dut_id"
    )
      ...
      kwargs,
      {
        "payload": {
          "body": {
            ...
            "executionTimeout": "1030s",
            "expiry": "2112-01-01T01:07:10Z",
            "requestedBotId": "pool:dut_id:dut1",
            "constraints": [
              {"key": "dim1", "allowedValues": ["val1"]},
              {"key": "dim1", "allowedValues": ["val2", "val3"]},
              {"key": "dim2", "allowedValues": ["val4"]},
  ...
  def test_enqueue_rbe_cancel(self, getrandbits, utcnow, enqueue_task):
    ...
    self.assertEqual(
      args, ("/internal/tasks/t/rbe-cancel/2ed6c6804c8002a10-2", "rbe-cancel")
    )
    self.assertEqual(
      kwargs,
      {
