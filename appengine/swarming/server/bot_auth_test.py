...
class BotAuthTest(test_case.TestCase):
  ...
  def test_not_ip_whitelisted(self):
    # Callers not in 'bots' IP whitelist are rejected right away, regardless
    # what the config says.
    self.mock_caller('anonymous:anonymous', '1.1.1.1')
    with self.assertRaises(auth.AuthorizationError):
      bot_auth.validate_bot_id_and_fetch_config('bot_with_ip_whitelist')
  ...
  def test_unknown_bot_id(self):
    ...
    self.mock_caller('anonymous:anonymous', '1.2.3.4')
    cfg = bot_auth.validate_bot_id_and_fetch_config('unknown_bot_id')
  ...
  def test_broken_config_section(self):
    ...
    self.mock_caller('anonymous:anonymous', '1.2.3.4')
    cfg = bot_auth.validate_bot_id_and_fetch_config('broken_config')
    self.assertIsNone(cfg)
