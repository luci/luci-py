...
BotDetails = collections.namedtuple(
  "BotDetails", ["bot_version", "logs_cloud_project"]
)
...
def _expire_slice_tx(
  request, to_run_key, terminal_state, culprit_bot_id, capacity, es_cfg
):
  ...
  to_run, result_summary = ndb.get_multi(
    [to_run_key, result_summary_key], use_cache=False, use_memcache=False
  )
...
def _expire_slice(
  ...
  if claim and not task_to_run.Claim.obtain(to_run_key):
    logging.warning(
      "%s/%s is marked as claimed, proceeding anyway",
      request.task_id,
      slice_index,
    )
...
def _reap_task(
  ...
  def run():
    ...
    if not to_run.is_reapable:
      if claim_id:
        ...
        run_result = task_pack.result_summary_key_to_run_result_key(
          result_summary_key
        ).get(use_cache=False, use_memcache=False)
        if not run_result:
    ...
    if result_summary.bot_id == bot_id:
      ...
      logging.warning(
        "%s can't retry its own internal failure task", result_summary.task_id
      )
      return None, None, None, None, False
...
def _detect_dead_task_async(run_result_key):
  ...
  def run():
    ...
    logging.warning(
      "Task state was successfully updated. task: %s", run_result.task_id
    )
    raise ndb.Return(
      run_result_key, state_changed, latency, result_summary.tags
    )
...
def _maybe_pubsub_notify_now(result_summary, request):
  ...
  if (
    result_summary.state not in task_result.State.STATES_RUNNING
...
def _maybe_pubsub_send_build_task_update(bb_task, build_id, pubsub_topic):
  ...
  try:
    pubsub.publish(
      topic=pubsub_topic, message=msg.SerializeToString(), attributes=None
    )
    ...
    logging.exception(
      "Unknown exception (%s) not handled by "
      "_maybe_pubsub_send_build_task_update",
      e,
    )
...
def _maybe_taskupdate_notify_via_tq(
  ...
  if request.pubsub_topic:
    if "projects/cr-buildbucket" in request.pubsub_topic or _route_to_go(
      prod_pct=100, dev_pct=100
    ):
      ...
      payload = {
        ...
        "body": {
          ...
          "state": TaskState.DESCRIPTOR.values_by_number[
            result_summary.state
          ].name,
      ...
      ok = utils.enqueue_task(
        "/internal/tasks/t/pubsub-go/%s" % task_id,
        "pubsub-v2",
    ...
    logging.debug(
      "Payload of PubSub msg that was tried to enqueued: %s", str(payload)
    )
    if not ok:
...
def _pubsub_notify(
  task_id, topic, auth_token, userdata, tags, state, start_time
):
  ...
  try:
    ...
    if start_time is not None:
      ...
      if latency < 0:
        ...
        latency = 0
      logging.debug(
        "Updating ts_mon_metric pubsub with latency: %dms (%d - %d)",
        latency,
        now,
        start_time,
      )
...
def _find_dupe_task(now, h):
  ...
  for i, dupe_summary in enumerate(q.iter(batch_size=1)):
    ...
    if (
      dupe_summary.state != task_result.State.COMPLETED or dupe_summary.failure
    ):
      if i == 2:
    ...
    if dupe_summary.created_ts <= oldest:
      logging.info(
        "found result (%s) is older than threshold (%s)",
        dupe_summary.created_ts,
        oldest,
      )
      return None
...
def _is_allowed_to_schedule(pool_cfg):
  ...
  if caller_id in pool_cfg.scheduling_users:
    logging.info(
      ...
      "specified directly in the pool config",
      caller_id.to_bytes(),
      pool_cfg.name,
    )
    return True
    ...
    if run_result.exit_code is not None:
      ...
      if run_result.exit_code != exit_code:
        return (
          None,
          None,
          "got 2 different exit_code; %s then %s"
      ...
      if run_result.duration != duration:
        return (
          None,
          None,
          "got 2 different durations; %s then %s"
...
def _cancel_task_tx(
  request, result_summary, kill_running, bot_id, now, es_cfg, run_result=None
):
...
def _get_task_from_external_scheduler(es_cfg, bot_dimensions):
  ...
  logging.info(
    "Determined request_key, result_key %s, %s", request_key, result_key
  )
...
def _ensure_active_slice(request, task_slice_index):
  ...
  def run():
    logging.debug(
      "_ensure_active_slice(%s, %d)", request.task_id, task_slice_index
    )
    ...
    if to_runs:
      if len(to_runs) != 1:
        logging.warning(
          "_ensure_active_slice: %s != 1 TaskToRunShards", len(to_runs)
        )
        return None, True
...
def _bot_reap_task_external_scheduler(bot_dimensions, bot_details, es_cfg):
  ...
  run_result, secret_bytes = _reap_task(
    bot_dimensions, bot_details, to_run.key, request
  )
  if not run_result:
    raise external_scheduler.ExternalSchedulerException(
...
def exponential_backoff(attempt_num):
  ...
  max_wait = config.settings().max_bot_sleep_time or 60.0
...
def check_schedule_request_acl_caller(pool_cfg):
  if not _is_allowed_to_schedule(pool_cfg):
    raise auth.AuthorizationError(
      ...
      "see pools.cfg" % (auth.get_current_identity().to_bytes(), pool_cfg.name)
    )
...
def schedule_request(
  ...
  if request_id:
    ...
    if task_req_to_id:
      return _get_task_result_summary_from_task_id(
        task_req_to_id.task_id, request
      )
  ...
  if not dupe_summary:
    ...
    for index in range(request.num_task_slices):
      ...
      if (
        request.rbe_instance
        or t.wait_for_capacity
  ...
  while True:
    attempt += 1
    try:
      if datastore_utils.transaction(
        txn, retries=3, xg=bool(task_req_to_id_key)
      ):
        break
      ...
      logging.warning(
        "Task ID collision: %s already exists, using %s instead",
        prev,
        result_summary.task_id,
      )
    except datastore_utils.CommitError:
  ...
  if dupe_summary:
    logging.debug(
      "New request %s reusing %s", result_summary.task_id, dupe_summary.task_id
    )
...
def bot_reap_task(bot_dimensions, queues, bot_details, deadline):
  ...
  if es_cfg:
    ...
    logging.info(
      "External scheduler did not reap any tasks, trying native scheduler."
    )
  ...
  try:
    ...
    for to_run in q:
      ...
      if es_cfg and not _should_allow_es_fallback(es_cfg, request):
        logging.debug(
          "Skipped es-owned request %s during es fallback", request.task_id
        )
        continue
      ...
      if not run_result:
        ...
        logging.info(
          "failed to reap: %s0", task_pack.pack_request_key(to_run.request_key)
        )
        continue
      ...
      if es_cfg:
        ...
        external_scheduler.notify_requests(
          es_cfg, [(request, run_result)], True, False
        )
      logging.debug(
        "TODO(crbug.com/1186759): expiration_ts %s", to_run.expiration_ts
      )
      return request, secret_bytes, run_result
    return None, None, None
  finally:
...
def bot_claim_slice(bot_dimensions, bot_details, to_run_key, claim_id):
  ...
  if not to_run.is_reapable:
    ...
    request, run_result = ndb.get_multi(
      [to_run.request_key, run_result_key], use_cache=False, use_memcache=False
    )
    if not run_result:
  ...
  run_result, secret_bytes = _reap_task(
    bot_dimensions, bot_details, to_run_key, request, claim_id, 3, False
  )
  return request, secret_bytes, run_result
...
def bot_update_task(
  ...
  if smry and smry.state != task_result.State.RUNNING:
    ...
    ndb.get_context()._clear_memcache(
      [result_summary_key, run_result_key]
    ).check_success()
...
def bot_terminate_task(run_result_key, bot_id, start_time, client_error):
  ...
  def run():
    ...
    if run_result.killing:
      ...
      if missing_cas:
        run_result.missing_cas = [
          task_request.CASReference.create_from_json(cas) for cas in missing_cas
        ]
    ...
    ts_mon_metrics.on_dead_task_detection_latency(
      result_summary.tags, latency, False
    )
    return None
...
def cancel_task(request, result_key, kill_running, bot_id):
  ...
  def run():
    ...
    succeeded, was_running = _cancel_task_tx(
...
def cron_abort_expired_task_to_run():
  ...
  def _enqueue_task(to_runs):
    payload = {
      "entities": [
        (ttr.task_id, ttr.shard_index, ttr.key.integer_id()) for ttr in to_runs
      ],
    }
...
def cron_handle_bot_died():
  ...
  try:
    ...
    now = utils.utcnow()
    logging.info(
      "cron_handle_bot_died time elapsed: %ss", (now - start).total_seconds()
    )
    if now > time_to_stop:
...
def task_expire_tasks(task_to_runs):
  ...
  try:
    ...
    logging.info(
      "Reenqueued %d tasks, expired %d, skipped %d",
      reenqueued,
      len(expired),
      skipped,
    )
