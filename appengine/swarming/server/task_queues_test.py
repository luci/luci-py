...
class TaskQueuesApiTest(test_env_handlers.AppTestBase):
  ...
  def _assert_bot(self, bot_id="bot1", dimensions=None):
    bot_dimensions = {
      "cpu": ["x86-64", "x64"],
      "id": [bot_id],
      "os": ["Ubuntu-16.04", "Ubuntu"],
      "pool": ["default"],
    }
  ...
  def _assert_task(self, dimensions=None):
    ...
    request = _gen_request(
      properties=_gen_properties(dimensions=dimensions) if dimensions else None
    )
  ...
  def test_or_dimensions_new_tasks(self):
    ...
    self.assertEqual(
      [787294789, 1873318153], task_queues.freshen_up_queues("bot1")
    )
    self.assertEqual([787294789], task_queues.freshen_up_queues("bot2"))
  ...
  def test_or_dimensions_same_hash(self):
    self._assert_bot(bot_id="bot1", dimensions={"os": ["v1"]})
    self._assert_bot(bot_id="bot2", dimensions={"os": ["v2"]})
    self._assert_bot(bot_id="bot3", dimensions={"os": ["v3"]})
  ...
  def test_bot_dimensions_to_flat(self):
    ...
    self.assertEqual(["a:bee", "a:c", "cee:zee"], actual)
  ...
  def test_dimensions_to_flat_duplicate_value(self):
    actual = task_queues.bot_dimensions_to_flat({"a": ["c", "c"]})
  ...
  def test_assert_bot_dimensions_changed(self):
    ...
    self._assert_bot(dimensions={"os": ["Amiga"]})
    ...
    self.mock_now(now)
    self._assert_bot(dimensions={"os": "Commodore"})
  ...
  def test_hash_or_dimensions(self):
    ...
    dim2 = (
      _gen_request(
        properties=_gen_properties(
          dimensions={"pool": ["pool-a"], "foo": ["xyz", "b|c|a"]}
        )
      )
      .task_slice(0)
      .properties.dimensions
  ...
  def test_expand_dimensions_to_flats(self):
    ...
    # Without OR
    actual = set(
      tuple(f) for f in expand({"foo": ["c", "a", "b"], "bar": ["x", "z", "y"]})
    )
    expected = {
      ("bar:x", "bar:y", "bar:z", "foo:a", "foo:b", "foo:c"),
    }
    self.assertEqual(actual, expected)
    # With OR
    actual = set(
      tuple(f) for f in expand({"foo": ["a|b|c", "def"], "bar": ["x|y", "z|w"]})
    )
    expected = {
      ("bar:w", "bar:x", "foo:a", "foo:def"),
      ("bar:w", "bar:x", "foo:b", "foo:def"),
      ("bar:w", "bar:x", "foo:c", "foo:def"),
      ("bar:w", "bar:y", "foo:a", "foo:def"),
      ("bar:w", "bar:y", "foo:b", "foo:def"),
      ("bar:w", "bar:y", "foo:c", "foo:def"),
      ("bar:x", "bar:z", "foo:a", "foo:def"),
      ("bar:x", "bar:z", "foo:b", "foo:def"),
      ("bar:x", "bar:z", "foo:c", "foo:def"),
      ("bar:y", "bar:z", "foo:a", "foo:def"),
      ("bar:y", "bar:z", "foo:b", "foo:def"),
      ("bar:y", "bar:z", "foo:c", "foo:def"),
    }
    # flats are sorted
    actual = set(
      tuple(f)
  ...
  def test_expiry_map(self):
    sets = [
      {"dimensions": ("a:b", "a:c"), "expiry": 1663040000},
      {"dimensions": ("a:d", "a:e"), "expiry": 1663041111},
    ]
  ...
  @parameterized.expand(
    [
      ({"pool": ["p"], "k": ["v1"]}, "pool:p:1496061212"),
      ({"pool": ["p"], "k": ["v1|v2"]}, "pool:p:3930364299"),
      ({"pool": ["a:b"], "k": ["v1"]}, "pool:a%3Ab:2715169585"),
      ({"id": ["b"], "pool": ["p"], "k": ["v1"]}, "bot:b:174937362"),
      ({"id": ["b"], "pool": ["p"], "k": ["v1|v2"]}, "bot:b:903320086"),
    ]
  )
  def test_assert_task_dimensions_async(self, task_dims, expected_sets_id):
    ...
    self.assertEqual(
      sets_ent.to_dict(),
      {
        "sets": [{"dimensions": dims} for dims in expected_sets],
      },
    )
    ...
    self.assertEqual(
      ...
      {
        "sets": [
          {"dimensions": dims, "expiry": _to_timestamp(stored_exp)}
          for dims in expected_sets
        ],
        "next_cleanup_ts": stored_exp + datetime.timedelta(minutes=5),
      },
    )
  ...
  def test_assert_task_dimensions_async_collision(self):
    ...
    self.assertEqual(
      info_key.get().to_dict(),
      {
        ...
        "next_cleanup_ts": stored_exp1 + datetime.timedelta(minutes=5),
      },
    )
    ...
    self.assertEqual(
      info_key.get().to_dict(),
      {
        ...
        "next_cleanup_ts": stored_exp3 + datetime.timedelta(minutes=5),
      },
    )
  ...
  def _create_task_dims_set(sets_id, *task_dims):
    ...
    ndb.transaction_async(
      lambda: task_queues._put_task_dimensions_sets_async(sets_id, exp_map)
    ).get_result()

  @staticmethod
  def _delete_task_dims_set(sets_id):
    ndb.transaction_async(
      lambda: task_queues._delete_task_dimensions_sets_async(sets_id)
    ).get_result()
  ...
  def test_assert_bot_dimensions_async(self):
    ...
    self.assertEqual(
      bot_matches.to_dict(),
      {
        "dimensions": ["dim:0", "id:bot-id", "pool:pool1", "pool:pool2"],
    ...
    assert_tq_task(
      {
        "bot_id": "bot-id",
        "rescan_counter": 1,
        "rescan_reason": "dims added [dim:0 id:bot-id pool:pool1 pool:pool2]",
      }
    )
    ...
    assert_tq_task(
      {
        "bot_id": "bot-id",
        "rescan_counter": 3,
        "rescan_reason": "dims added [dim:1], dims removed [dim:0]",
      }
    )
    ...
    self.assertEqual(
      bot_matches.to_dict(),
      {
        "dimensions": ["dim:1", "id:bot-id", "pool:pool1", "pool:pool2"],
        "last_addition_ts": None,
        "last_cleanup_ts": now,
        "last_rescan_enqueued_ts": now,
        "last_rescan_finished_ts": None,  # we mocked this out
        "matches": ["bot:bot-id:1", "pool:pool1:2", "pool:pool2:4"],
        "next_rescan_ts": now + datetime.timedelta(minutes=30),
        "rescan_counter": 3,
      },
    )
    ...
    self.assertEqual(
      bot_matches.to_dict(),
      {
        ...
        "last_rescan_finished_ts": None,  # we mocked this out
        "matches": ["bot:bot-id:1", "pool:pool1:2"],
  ...
  def test_tq_rescan_matching_task_sets_async(self):
    ...
    task_queues.BotDimensionsMatches(
      ...
      matches=[
        "bot:bot-id:1",
        "bot:bot-id:2",
        "bot:bot-id:444",  # missing, should be unmatched
        "pool:missing:555",  # missing, should be unmatched
        "pool:pool1:4",
        "pool:pool1:5",
        "pool:pool2:7",
        "pool:pool2:8",
    ...
    # Run the rescan.
    self.assertTrue(
      task_queues._tq_rescan_matching_task_sets_async(
        "bot-id", 555, "reason"
      ).get_result()
    )
  ...
  def test_tq_update_bot_matches_async_pool(self):
    ...
    self.assertTrue(
      task_queues._tq_update_bot_matches_async(
        task_sets_id, task_sets_dims, now
      ).get_result()
    )
  ...
  def test_tq_update_bot_matches_async_skip(self):
    ...
    def call(sets_id, tasks_dims):
      return task_queues._tq_update_bot_matches_async(
        sets_id, tasks_dims, now
      ).get_result()
