...
def get_rbe_config_for_bot(bot_id, pools):
  ...
  for pool in pools:
    ...
    if cfg and cfg.rbe_migration:
      ...
      per_pool.append(
        (
          mode,
          cfg.rbe_migration.rbe_instance,
  ...
  if len(rbe_instances) > 1:
    logging.error(
      "RBE: bot %s: bot pools disagree on RBE instance: %r",
      bot_id,
      rbe_instances,
    )
...
def enqueue_rbe_task(task_request, task_to_run):
  ...
  for k, v in sorted(props.dimensions.items()):
    ...
    if "id" == k:
      ...
      assert len(v) == 1, (
        "expect exactly one value for dimension %s, got %r"
        % (effective_bot_id_dimension, v)
      )
      if task_request.pool:
        effective_bot_id = "%s:%s:%s" % (
          task_request.pool,
          effective_bot_id_dimension,
          v[0],
        )
  ...
  if (
    effective_bot_id
    and effective_bot_id_from_bot
    and (effective_bot_id != effective_bot_id_from_bot)
  ):
    logging.error(
