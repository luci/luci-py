...
def process_auth_params_json(val):
  ...
  if not (exp is None or isinstance(exp, int)):
    raise ValueError(
      'Expecting "swarming_http_headers_exp" to be int or None, got %r' % (exp,)
    )
...
class AuthSystem(object):
  ...
  def _check_and_log_token(self, tok, account_id, service_account):
    ...
    if tok.expiry - time.time() < 0:
      raise auth_server.RPCError(
        500,
        (
          "The new %r token (belonging to %r) has already expired (%d vs "
  ...
  def _grab_oauth_token_via_rpc(
    self, auth_params, rpc_client, account_id, scopes
  ):
  ...
  def _grab_id_token_via_rpc(
    self, auth_params, rpc_client, account_id, audience
  ):
    ...
    with self._rpc_guard(lock_key=("id_token", account_id, audience)):
      resp = rpc_client.mint_id_token(
        task_id=auth_params.task_id, account_id=account_id, audience=audience
      )
