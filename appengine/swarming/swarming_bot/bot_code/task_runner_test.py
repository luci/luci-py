...
def run_command(server_url, work_dir, task_details, headers_cb, rbe_session):
  ...
  remote = remote_client.RemoteClientNative(
    server_url, headers_cb, task_details.bot_id, work_dir
  )
...
def load_and_run(
  server_url, work_dir, manifest, auth_params_file, rbe_session=None
):
...
class TestTaskRunnerBase(auto_stub.TestCase):
  ...
  def getTaskResults(self, task_id):
    ...
    if "output" in actual:
      actual["output"] = cas_util.filter_out_go_logs(
        base64.b64decode(actual["output"]).decode()
      ).encode()
  ...
  def _run_command(self, task_details, rbe_session=None):
    return run_command(
      self.server.url, self.work_dir, task_details, None, rbe_session
    )
...
class TestTaskRunner(TestTaskRunnerBase):
  ...
  def test_run_command_env_prefix_multiple(self):
    ...
    if sys.platform == "win32":
      output = re.compile(output.pattern.replace(rb"\n", rb"\r\n"))
  ...
  def test_run_command_os_error(self):
    ...
    pattern = (
      ...
      # found.
      rb"(\S|\s)*"
      rb"<The executable does not exist, a dependent library is missing or "
      rb"the command line is too long>\n"
      rb"<Check for missing .so/.dll in the .isolate or GN file or length of "
      rb"command line args>"
    )
    if sys.platform == "win32":
      pattern = pattern.replace(rb"\n", rb"\r\n")
    output = re.compile(pattern)
    out = self.expectTask(task_details.task_id, exit_code=1, output=output)
    self.assertGreater(10.0, out["cost_usd"])
  ...
  def test_run_command_large(self):
    ...
    self.assertEqual(
      base64.b64encode(to_native_eol("hi!\n" * 100002).encode()),
      updates[1]["output"].encode(),
    )
    self.assertEqual(
      base64.b64encode(to_native_eol("hi!\n").encode()),
      updates[2]["output"].encode(),
    )
  ...
  def test_main(self):
    ...
    def _load_and_run(
      ...
      self.assertEqual(3600.0, cost_usd_hour)
...
class TestTaskRunnerKilled(TestTaskRunnerBase):
  ...
  def test_hard_signal(self):
    ...
    self.expectTask(
      task_details.task_id, hard_timeout=True, output=expected_output_re
    )
  ...
  def test_io_signal(self):
    ...
    self.expectTask(
      task_details.task_id, io_timeout=True, output=expected_output_re
    )
  ...
  def test_signal(self):
    ...
    remote_client.SessionState(
      session_id="fake-session", session_token="fake-token", rbe_session=None
    ).dump(session_state_file)
    ...
    expected = {
      manifest["task_id"]: [
        {
          "message": "task_runner received signal %d"
          % task_runner.SIG_BREAK_OR_TERM,
          "client_error": {
            "missing_cas": [],
            "missing_cipd": [],
...
class TaskRunnerNoServer(auto_stub.TestCase):
  ...
  def test_load_and_run_isolated(self):
    ...
    def _run_command(
      ...
      self.assertGreaterEqual(time.time(), start)
      self.assertEqual(
        ["--min-free-space", "1"] + DISABLE_CIPD_FOR_TESTS, run_isolated_flags
      )
    ...
    actual = load_and_run(
      "http://localhost:1", self.root_dir, manifest, "/path/to/auth-params-file"
    )
    expected = {
      "exit_code": 0,
      "hard_timeout": False,
  ...
  def test_load_and_run_raw(self):
    ...
    try:
      self.mock(bot_auth, "AuthSystem", FakeAuthSystem)
      with mock.patch(
        "%s.TaskDetails.load" % task_runner.__name__,
  ...
  def test_load_and_run_raw_with_bb_agent_ctx_ok(self):
    ...
    task_details = get_task_details(
      command=["bbagent", "-context-file", "${BUILDBUCKET_AGENT_CONTEXT_FILE}"],
      task_id="4b23423f1",
      realm=realm_ctx,
