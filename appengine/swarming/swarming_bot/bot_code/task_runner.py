...
def get_isolated_args(
  work_dir, task_details, isolated_result, bot_file, run_isolated_flags
):
...
class TaskDetails:
  ...
  def __init__(self, data):
    ...
    for k in self._REQUIRED:
      if k not in data:
        raise InternalError(
          "Task manifest missing required key %s:\n%r" % (k, data)
        )
...
def tmp_bb_agent_context_file(secret_bytes, task_id, workdir):
  ...
  content = launcher_pb2.BuildbucketAgentContext(
    task_id=task_id, secrets=secrets
  )
  ...
  try:
    with subprocess42.set_signal_handler([SIG_BREAK_OR_TERM], handler):
      ...
      remote = remote_client.RemoteClientNative(
        swarming_server, headers_cb, task_details.bot_id, work_dir
      )
      remote.initialize()
      ...
      if "${BUILDBUCKET_AGENT_CONTEXT_FILE}" in task_details.command:
        bb_ctx_tf = tmp_bb_agent_context_file(
          task_details.secret_bytes, result_summary_id, work_dir
        )
    ...
    if bb_ctx_tf:
      try:
        ...
        logging.exception(
          "could not close buildbucket context file. %s" % str(ex)
        )
...
def fail_without_command(
  remote, task_id, params, cost_usd_hour, task_start, exit_code, stdout
):
  now = monotonic_time()
...
def _start_task_runner(args, work_dir, ctx_file):
  ...
  try:
    ...
    raise _FailureOnStart(
      e.errno or -1, "Could not write args to %s: %s" % (args_path, e)
    )
...
class _OutputBuffer:
  ...
  def should_post_update(self):
    ...
    """
    packet_interval = (
      self._min_packet_interval if self._stdout else self._max_packet_interval
    )
    return (
      len(self._stdout) >= self._max_chunk_size
      or (self._last_loop - self._last_pop) > packet_interval
    )
  ...
  def calc_yield_wait(self, timed_out):
    ...
    if timed_out:
      ...
      if self._task_details.grace_period:
        return max(
          self._last_loop - timed_out - self._task_details.grace_period, 0.0
        )
      return 0.0
...
class _RBEPinger:
  ...
  def ping(self):
    ...
    if self._rbe_session.alive and self._rbe_session.terminating:
      logging.warning(
        "RBE session %s is pending termination", self._rbe_session.session_id
      )
  ...
  def _ping_session(self):
    if not self._rbe_session.alive:
      logging.warning(
        "RBE session %s is dead, attempting to recreate",
    ...
    try:
      ...
      if not self._rbe_session.alive:
        logging.warning(
          "RBE session %s is dead and will be recreated later",
          self._rbe_session.session_id,
        )
    except remote_client.RBEServerError as e:
...
def run_command(
  ...
  params = {
    "cost_usd": cost_usd_hour * (start - task_start) / 60.0 / 60.0,
  }
  ...
  try:
    ...
    try:
      ...
      for channel, new_data in proc.yield_any(
        maxsize=buf.maxsize, timeout=yield_timeout
      ):
        ...
        if buf.should_post_update():
          params["cost_usd"] = (
            cost_usd_hour * (monotonic_time() - task_start) / 60.0 / 60.0
          )
          if not remote.post_task_update(
            task_details.task_id, params, buf.pop()
          ):
            logging.debug(
              "Server induced stop; kill_sent: %s, term_sent: %s",
              kill_sent,
              term_sent,
            )
        ...
        if not timed_out:
          if (
            task_details.io_timeout
            and buf.since_last_io > task_details.io_timeout
          ):
            had_io_timeout = True
            if not term_sent:
              logging.warning(
          ...
          if (
            not kill_sent
            and buf.last_loop - timed_out >= task_details.grace_period
          ):
      ...
      if not kill_sent:
        logging.info(
          "got exit code %d, but kill whole process group", exit_code
        )
        proc.kill()
    except (
      ExitSignal,
    ...
    try:
      if internal_error:
        ...
        logging.debug("internal_error: True. Resetting exit_code and params.")
        exit_code = None
      ...
      remote.post_task_update(
        task_details.task_id, params, buf.pop(), exit_code
      )
      logging.debug(
        "Last task update finished. task_id: %s, exit_code: %s, params: %s.",
        task_details.task_id,
        exit_code,
        params,
      )
      if internal_error:
        remote.post_task_error(
          task_details.task_id,
          internal_error,
...
def main(args):
  ...
  parser.add_option(
    "--session-state-file", help="Path to a file with Swarming session state"
  )
