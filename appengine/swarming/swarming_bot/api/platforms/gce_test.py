...
class TestGCE(auto_stub.TestCase):
  _METADATA = {
    "project": {"projectId": "test-proj", "numericProjectId": 123456},
    "instance": {
      ...
      "hostname": "host.name",
      "machineType": "projects/123456/machineTypes/n1-standard-8",
      "description": "",
      "zone": "projects/123456/zones/us-fakegcezone1-a",
      "maintenanceEvent": "NONE",
      "image": "projects/test-proj/global/images/test-image",
      "disks": [
        {
          "interface": "SCSI",
      ...
      "serviceAccounts": {
        "default": {
          ...
          "email": "test@test-proj.iam.gserviceaccount.com",
          "aliases": ["default"],
        },
  ...
  def test_get_cpuinfo(self):
    self.assertEqual(
      gce.get_cpuinfo_uncached(),
      {"name": "Intel(R) Xeon(R) CPU Broadwell GCE", "vendor": "GenuineIntel"},
    )

  def test_get_cpuinfo_amd(self):
    self.mock_get_metadata.return_value = {
      "instance": {"cpuPlatform": "AMD Rome"}
    }
    self.assertEqual(
      gce.get_cpuinfo_uncached(),
      {"name": "AMD Rome GCE", "vendor": "AuthenticAMD"},
    )
  ...
  def test_get_cpuinfo_ampere(self):
    ...
    self.assertEqual(
      gce.get_cpuinfo_uncached(), {"name": "Ampere Altra GCE", "vendor": "ARM"}
    )
  ...
  def test_get_cpuinfo_google_axion(self):
    ...
    self.assertEqual(
      gce.get_cpuinfo_uncached(), {"name": "Google Axion GCE", "vendor": "ARM"}
    )
...
class TestMetadata(auto_stub.TestCase):
  ...
  def test_get_metadata(self):
    ...
    with mock.patch(
      "api.platforms.gce.get_metadata_uncached"
...
@unittest.skipUnless(
  gce.is_gce(), "TestOauth2AccessToken runs only on GCE machines"
)
...
class TestSignedMetadataToken(auto_stub.TestCase):
  ...
  def test_works(self):
    ...
    jwt = (
      b"unimportant."
      + base64.urlsafe_b64encode(bytes_json).rstrip(b"=")
      + b".unimportant"
    )
