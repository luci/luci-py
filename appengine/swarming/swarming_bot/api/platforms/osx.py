...
def _SMC_get_value(conn, key):
  ...
  if t == "sp78\0" and val.size == 2:
    ...
    return round(
      float(ctypes.c_int8(val.bytes[0]).value) + (val.bytes[1] / 256.0), 2
    )
  ...
  if os.path.exists(xcodebuild):
    try:
      out = subprocess.check_output(
        [xcodebuild, "-version"], universal_newlines=True
      ).splitlines()
...
def get_ios_device_ids():
  ...
  try:
    return subprocess.check_output(
      ["idevice_id", "--list"], universal_newlines=True
    ).splitlines()
  except (OSError, subprocess.CalledProcessError):
    return []
...
def get_ios_device_type(udid):
  ...
  try:
    out = subprocess.check_output(
      ["ideviceinfo", "-k", "ProductType", "-u", udid], universal_newlines=True
    ).splitlines()
    if len(out) == 1:
      return out[0]
  except (OSError, subprocess.CalledProcessError):
...
def get_hardware_model_string():
  ...
  try:
    return subprocess.check_output(
      ["sysctl", "-n", "hw.model"], universal_newlines=True
    ).rstrip()
  except (OSError, subprocess.CalledProcessError):
    return None
...
def get_os_build_version():
  ...
  """
  out = subprocess.check_output(
    ["sw_vers", "-buildVersion"], universal_newlines=True
  ).splitlines()
...
def _handle_non_apple_gpu(card, dimensions, state):
  ...
  if version:
    ...
    if match:
      dimensions.add(
        "%s:%s-%s-%s" % (ven_id, dev_id, match.group(1), match.group(2))
      )
...
def get_cpuinfo():
  ...
  values = common._safe_parse(
    subprocess.check_output(["sysctl", "machdep.cpu"], universal_newlines=True)
  )
  # http://unix.stackexchange.com/questions/43539/what-do-the-flags-in-proc-cpuinfo-mean
  info = {
    "flags": sorted(
      i.lower() for i in values.get("machdep.cpu.features", "").split()
    ),
    "name": values.get("machdep.cpu.brand_string", "Unknown"),
    "vendor": values.get("machdep.cpu.vendor", "Unknown"),
  }
...
def get_monitor_hidpi():
  ...
  def is_hidpi(displays):
    return any(
      d.get("spdisplays_retina") == "spdisplays_yes"
      or "retina" in d.get("spdisplays_display_type", "").lower()
      for d in displays
    )
...
def get_physical_ram():
  ...
  return int(round(result.value / 1024.0 / 1024.0))
...
def get_uptime():
  ...
  start = float(result.tv_sec) + float(result.tv_usec) / 1000000.0
...
def generate_launchd_plist(command, cwd, plistname):
  ...
  entries.append(
    "<key>Program</key><string>%s</string>" % saxutils.escape(command[0])
  )
  ...
  entries.append("</array>")
  entries.append(
    "<key>WorkingDirectory</key><string>%s</string>" % saxutils.escape(cwd)
  )
  header = (
