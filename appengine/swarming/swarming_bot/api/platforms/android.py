...
def get_unknown_apps(device):
  return [
    p
    for p in device.GetPackages() or []
    if (
      not p.startswith(("com.android.", "com.google.")) and p not in KNOWN_APPS
    )
  ]
...
def get_devices(
  bot=None, endpoints=None, enable_resets=False, allow_missing_keys=False
):
...
def get_dimensions(devices):
  ...
  # build IDs.
  if dimensions.get("device_os") and all(
    v.lower() in ["main", "master"] for v in dimensions["device_os"]
  ):
    os_prop_names = [
    ...
    android_vers = {
      os[0] for os in dimensions.get("device_os", []) if os and os[0].isupper()
    }
  ...
  dimensions["device_gms_core_version"] = _get_package_versions(
    "com.google.android.gms"
  ) or ["unknown"]
  dimensions["device_playstore_version"] = _get_package_versions(
    "com.android.vending"
  ) or ["unknown"]
...
def get_state(devices):
  ...
  def fn(device):
    ...
    return {
      ...
      "build": {key: properties.get("ro." + key, "<missing>") for key in keys},
      "cpu": device.GetCPUScale(),
      "disk": device.GetDisk(),
      "imei": device.GetIMEI(),
      "ip": device.GetIPs(),
      "max_uid": device.GetLastUID(),
      "mem": device.GetMemInfo(),
      "other_packages": get_unknown_apps(device),
      "port_path": device.port_path,
      "processes": device.GetProcessCount(),
      "state": (
        "available" if no_sd_card or device.IsFullyBooted()[0] else "booting"
      ),
      "temp": device.GetTemperatures(),
