    ...
    self.assertEqual(
      {
        ...
        "model": (0, 3087, 4),
        "name": "SAMSUNG EXYNOS5",
        "revision": "0000",
        "serial": "",
        "vendor": "ARMv7 Processor rev 4 (v7l)",
      },
...
class TestLinux(auto_stub.TestCase):
  ...
  def test_get_gpu_intel(self):
    ...
    self.mock_check_output.side_effect = (
      # lscpi -mm -nn
      '00:02.0 "VGA compatible controller [0300]" "Intel Corporation [8086]" "Device [9bc5]" -r05 "Dell [1028]" "Device [09a4]"'.encode(
        "utf-8"
      ),
  ...
  def test_get_gpu_amd(self):
    ...
    self.mock_check_output.side_effect = (
      # lscpi -mm -nn
      '03:00.0 "VGA compatible controller [0300]" "Advanced Micro Devices, Inc. [AMD/ATI] [1002]" "Navi 14 [Radeon RX 5500/5500M / Pro 5500M] [7340]" -rc5 "Gigabyte Technology Co., Ltd [1458]" "Navi 14 [Radeon RX 5500/5500M / Pro 5500M] [2319]"'.encode(
        "utf-8"
      ),
      # dpkg -s libgl1-mesa-dri
      "Version: 23.2.1-1ubuntu3.1~22.04.2".encode("utf-8"),
  ...
  def test_get_device_tree_compatible(self):
    with mock.patch("builtins.open") as mock_open:
      ...
      self.assertEqual(
        linux.get_device_tree_compatible(), sorted(["foo", "bar"])
      )
...
"""
  ...
  def test_is_display_attached_unknown(self):
    ...
    self.assertIn(
      "ERROR:root:is_display_attached(): Executable not found",
...
"""
  ...
  def test_get_display_resolution_command_error(self):
    ...
    with self.assertLogs(level="ERROR") as logging_manager:
      self.assertIsNone(linux.get_display_resolution())
    self.assertIn(
      "ERROR:root:get_display_resolution(): Executable not found",
      logging_manager.output,
    )
