...
@unittest.skipUnless(
  sys.platform == "win32", "Tests only run under Windows platform"
)
class TestWin(auto_stub.TestCase):
  ...
  def assert_get_os_dims_mock(
    self, product_type_int, cmd_ver_out, win32_ver_out, expected_version_names
  ):
  ...
  def test_get_gpu(self):
    SWbemObjectSet = mock.Mock(
      PNPDeviceID="PCI\\VEN_1AE0&DEV_A002&SUBSYS_00011AE0&REV_01\\3&13C0B0C5&0&28",
      VideoProcessor="GGA",
    ...
    with mock.patch(
      ...
      expected = (
        ["1ae0", "1ae0:a002", "1ae0:a002-1.1.1.18"],
        ["Unknown GGA 1.1.1.18"],
      )
  ...
  def test_get_gpu_qualcomm(self):
    ...
    with mock.patch(
      ...
      expected = (
        ["qcom", "qcom:043a", "qcom:043a-27.20.1870.0"],
  ...
  def test_get_screen_scaling_percent_success(self):
    ...
    for physical, logical, result in cases:
      with mock.patch(
        "api.platforms.win.is_display_attached", return_value=True
      ):
...
class TestWinPlatformIndendent(auto_stub.TestCase):
  ...
  def test_get_screen_scaling_percent_no_display(self):
    with mock.patch(
      "api.platforms.win.is_display_attached", return_value=False
    ):
