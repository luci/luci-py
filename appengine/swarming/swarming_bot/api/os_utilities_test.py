...
class TestOsUtilities(auto_stub.TestCase):
  ...
  def test_get_os_values_linux(self):
    with (
      ...
      mock.patch("%s.get_os_name" % os_utilities.__name__, lambda: "Ubuntu"),
    ):
      self.assertEqual(
  ...
  def test_get_os_values_mac(self):
    with (
      mock.patch("platforms.osx.get_os_version_number", return_value="10.15.5"),
      mock.patch("platforms.osx.get_os_build_version", return_value="19F101"),
    ):
      self.assertEqual(
        os_utilities.get_os_values(),
  ...
  def test_get_cpu_dimensions_mips(self):
    ...
    self.mock(
      os_utilities, "get_cpuinfo", lambda: {"name": "Cavium Octeon II V0.1"}
    )
  ...
  def test_get_disks_info(self):
    ...
    # Round the same way.
    free_disk = round(
      float(file_path.get_free_space(root_path)) / 1024.0 / 1024.0, 1
    )
    ...
    self.assertLess(delta, 2.0, (delta, free_disk, root["free_mb"]))
  ...
  def test_get_os_version_parts(self):
    ...
    self.assertEqual(
      os_utilities.get_os_version_parts(version, os_name), expected
    )
