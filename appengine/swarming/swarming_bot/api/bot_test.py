...
class TestBot(unittest.TestCase):
  def test_get_pseudo_rand(self):
    ...
    self.assertEqual(
      -1.0, round(struct.unpack("h", b"\x00\x80")[0] / 32768.0, 4)
    )
    self.assertEqual(
      1.0, round(struct.unpack("h", b"\xff\x7f")[0] / 32768.0, 4)
    )
    b = make_bot()
    self.assertEqual(-0.7782, b.get_pseudo_rand(1.0))
    self.assertEqual(-0.0778, b.get_pseudo_rand(0.1))
  ...
  def test_attribute_updates(self):
    ...
    self.assertEqual(
      {"id": ["bot1"], "foo": ["baz"], "pool": ["A"]}, obj.dimensions
    )
