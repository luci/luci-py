...
class TestMessageConversion(test_case.TestCase):
  ...
  def _create_default_new_task_request_proto(self):
    ntr = swarming_pb2.NewTaskRequest(
  ...
  def _create_default_task_slice_proto(self, arg="arg"):
    ...
    props.cipd_input.client_package.package_name = (
    ...
    props.env_prefixes.extend(
      [
        swarming_pb2.StringListPair(key="foo", value=["bar", "baz"]),
      ]
    )
    props.env.extend([swarming_pb2.StringPair(key="foo", value="bar")])
    props.cas_input_root.cas_instance = (
      "projects/chromium-swarm-dev/instances/default_instance"
    )
    props.cas_input_root.digest.hash = (
      b"d00b122490e024587990b55c248dc82d346c54d89019d6eff61ddc9743ea0442"
    )
      ...
      properties=task_request.TaskProperties(
        ...
        cas_input_root=task_request.CASReference(
          ...
          digest=task_request.Digest(
            hash=(
              b"d00b122490e024587990b55c248dc82d346c54"
        ...
        command=["python", "run_test.py", arg],
        containment=task_request.Containment(containment_type=2),
        dimensions_data={"os": ["Amiga"], "pool": ["default"]},
        env={"foo": "bar"},
        env_prefixes={"foo": ["bar", "baz"]},
  ...
  def test_that_duplicate_env_variables_throws_error(self):
    ...
    with self.assertRaises(ValueError) as ctx:
      message_conversion_prpc.new_task_request_from_rpc(ntr)
    self.assertTrue(
      "same environment variable key cannot be specified twice"
      in str(ctx.exception)
    )
  ...
  def test_that_duplicate_env_prefixes_throws_error(self):
    ...
    with self.assertRaises(ValueError) as ctx:
      message_conversion_prpc.new_task_request_from_rpc(ntr)
    self.assertTrue(
      "same environment prefix key cannot be specified twice"
      in str(ctx.exception)
    )
  ...
  def test_bot_ping_tolerance_is_filled_correctly(self):
    ...
    self.assertEqual(
      actual.bot_ping_tolerance_secs, task_request.DEFAULT_BOT_PING_TOLERANCE
    )
