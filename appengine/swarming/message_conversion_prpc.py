...
def bot_info_response(bot_info, deleted=False):
  ...
  return swarming_pb2.BotInfo(
    ...
    bot_id=bot_info.id,
    dimensions=_string_list_pairs_from_dict(bot_info.dimensions),
    deleted=deleted,
...
def _cas_reference(ref):
  ...
  if ref is None:
    return None
  return swarming_pb2.CASReference(
    cas_instance=ref.cas_instance,
    digest=swarming_pb2.Digest(
      hash=ref.digest.hash, size_bytes=ref.digest.size_bytes
    ),
  )
...
def _resultdb_info(info):
  ...
  return swarming_pb2.ResultDBInfo(
    hostname=info.hostname, invocation=info.invocation
  )
...
def task_result_response(result, include_performance_stats=True):
  ...
  out = swarming_pb2.TaskResultResponse(
    ...
    if include_performance_stats and result.performance_stats.is_valid
    else None,
...
def _task_properties(props):
  ...
  return swarming_pb2.TaskProperties(
    ...
    env=[swarming_pb2.StringPair(key=k, value=v) for k, v in props.env.items()]
    if props.env
    else None,
...
def _task_properties_from_rpc(props):
  ...
  if props.HasField("cas_input_root"):
    if (
      props.cas_input_root.HasField("digest")
  ...
  if request.HasField("properties"):
    ...
    slices = [
      task_request.TaskSlice(
        properties=props, expiration_secs=request.expiration_secs
      ),
    ]
