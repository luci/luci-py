...
def find_proto_files(path):
  ...
  for dirpath, dirnames, filenames in os.walk(path, followlinks=True):
    ...
    skipped = [
      x for x in dirnames if x[0] == "." or is_ignored(os.path.join(dirpath, x))
    ]
...
def install_protoc():
  ...
  return os.path.join(
    root, "bin", "protoc.exe" if sys.platform == "win32" else "protoc"
  )
...
def check_proto_compiled(protoc, proto_file, proto_path):
  ...
  try:
    try:
      compiled = compile_proto(
        protoc, proto_file, proto_path, output_path=tmp_dir
      )
...
def compile_all_files(protoc, root_dir, proto_path):
  ...
  for path in find_proto_files(root_dir):
    try:
      ...
      print(
        "Failed to compile: %s" % path[len(root_dir) + 1 :], file=sys.stderr
      )
      success = False
  return success
