...
def bootstrap(paths, global_env=None, filters=None):
  ...
  if global_env is not None:
    ...
    assert all(_GLOBAL_ENV.get(k, v) == v for k, v in global_env.items()), (
      global_env
    )
  ...
  if filters is not None:
    ...
    assert all(
      isinstance(k, str) and callable(v) for k, v in filters.items()
    ), filters
    assert all(_GLOBAL_FILTERS.get(k, v) == v for k, v in filters.items()), (
      filters
    )
