...
class EndpointsFlaskTestCase(test_case.TestCase):
  def test_decode_message_post(self):
    ...
    msg = adapter.decode_message(
      EndpointsService.post.remote, request, request_kwargs
    )
  ...
  def test_decode_message_get(self):
    ...
    request.form = datastructures.MultiDict(
      [("s2", "b"), ("r", "x"), ("r", "y")]
    )
  ...
  def test_handle_403(self):
    ...
    self.assertEqual(
      json.loads(response.data), {"error": {"message": "access denied"}}
    )
  ...
  def test_api_routes(self):
    routes = sorted(
      [
        route
        for route, _, _, _ in adapter.api_routes(
          [EndpointsService], base_path="/_ah/api"
        )
      ]
    )
    self.assertEqual(
