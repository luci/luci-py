    ...
    if flask.request.values.get("fields"):
      try:
        ...
        logging.warning(
          ...
          flask.request.values.get("fields"),
          e,
        )
...
def api_routes(api_classes, base_path="/_ah/api"):
  ...
  for api_class in api_classes:
    ...
    path_version = (
      ...
      if hasattr(api_info, "path_version")
      else api_info.version
    )
...
def explorer_proxy_route(base_path):
  ...
  def proxy_handler():
    ...
    return template.render(
      "endpoints_adapter/proxy.html", params={"base_path": base_path}
    )
