...
def snapshot_to_dict(snapshot):
  ...
  result = {
    ...
    "ip_whitelist_assignments": entity_to_dict(
      snapshot.ip_whitelist_assignments
    ),
...
class NewAuthDBSnapshotTest(test_case.TestCase):
  ...
  def test_non_empty(self):
    ...
    permissionslist_blob = permissions_pb2.PermissionsList(
      permissions=[
...
class ShardedAuthDBTest(test_case.TestCase):
  ...
  def test_works(self):
    ...
    shard_ids = replication.store_sharded_auth_db(
      auth_db, self.PRIMARY_URL, self.AUTH_DB_REV, 50
    )
    ...
    for shard_id in shard_ids:
      ...
      shard = model.snapshot_shard_key(
        self.PRIMARY_URL, self.AUTH_DB_REV, shard_id
      ).get()
      self.assertTrue(len(shard.blob) <= 50)

    # Verify it can be reassembled back.
    reassembled = replication.load_sharded_auth_db(
      self.PRIMARY_URL, self.AUTH_DB_REV, shard_ids
    )
