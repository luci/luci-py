...
class GetAccessTokenTest(test_case.TestCase):
  ...
  def test_memcache_key(self):
    cache_key = service_account._memcache_key(
      method="pkey", email="blah@example.com", scopes=["1", "2"], key_id="abc"
    )
    self.assertEqual(
  ...
  def test_good_key_is_used(self):
    ...
    self.assertEqual(
      ("token", 0), service_account.get_access_token("scope", FAKE_SECRET_KEY)
    )
  ...
  def test_get_access_token_async(self):
    ...
    def get_access_token_async_mock(
      ...
      result = yield orig_get_access_token_async(
        scopes, service_account_key, act_as, min_lifetime_sec
      )
      raise ndb.Return(result)

    # Wrap get_access_token to mock out local signing
    self.mock(
      service_account, "get_access_token_async", get_access_token_async_mock
    )
    ...
    self.assertEqual(
      ...
      service_account.get_access_token_async(
        ["a", "b"], service_account_key=FAKE_SECRET_KEY
      ).get_result(),
    )

    res = service_account.get_access_token_async(
      ["c"], service_account_key=None, act_as="foo@example.com"
    ).get_result()
    self.assertEqual(
      (
        "foobartoken",
        int(
          utils.datetime_to_timestamp(utils.parse_rfc3339_datetime(expire_time))
          / 1e6
        ),
      ),
      res,
