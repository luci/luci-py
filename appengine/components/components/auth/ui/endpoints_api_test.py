...
class MembershipTest(test_case.EndpointsTestCase):
  ...
  def add_group(self, group, identities):
    model.AuthGroup(
      ...
      members=[model.Identity.from_bytes(identity) for identity in identities],
    ).put()
  ...
  def test_is_member_ok(self):
    ...
    self.assertEqual({"is_member": False}, response.json)
    ...
    response = self.call_api("membership", msg_dict(request), 200)
    self.assertEqual({"is_member": True}, response.json)
  ...
  def test_is_member_fails_with_degenerate_request(self):
    requests = [
      endpoints_api.MembershipRequest.combined_message_class(group="testgroup"),
