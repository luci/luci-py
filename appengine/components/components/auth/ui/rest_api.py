...
def get_rest_api_routes():
  ...
  return [
    ...
    webapp2.Route(
      "/auth/api/v1/listing/groups/<name:%s>" % group_re, GroupListingHandler
    ),
...
class EntityHandlerBase(handler.ApiHandler):
  ...
  def post(self, name):
    ...
    self.send_response(
      ...
      headers={
        ...
        "Location": "%s%s"
        % (self.entity_url_prefix, urllib.parse.quote(entity.key.id())),
      },
    )
  ...
  def put(self, name):
    ...
    def update(params, expected_ts):
      ...
      if (
        expected_ts
        and utils.datetime_to_rfc2822(entity.modified_ts) != expected_ts
      ):
        return (
          None,
          None,
          {
            "http_code": 412,
            "text": "%s was modified by someone else"
    ...
    def delete(expected_ts):
      ...
      if not entity:
        if expected_ts:
          return None, {
            "http_code": 412,
            "text": "%s was deleted by someone else"
            % self.entity_kind_title.capitalize(),
          }
...
class SelfHandler(handler.ApiHandler):
  ...
  api_doc = [
    {
      "verb": "GET",
      "doc": "Returns identity of a caller based on passed authentication tokens, "
  ...
  def get(self):
    ...
    if auth_db_rev:
      try:
        ...
        self.abort_with_error(400, text="Invalid 'auth_db_rev' param: %s" % exc)
...
class GroupHandler(EntityHandlerBase):
  ...
  def do_create(cls, entity):
    ...
    if missing:
      raise EntityOperationError(
        message="Some referenced groups don't exist: %s." % ", ".join(missing),
  ...
  def do_delete(cls, entity):
    ...
    if referencing_groups:
      ...
      raise EntityOperationError(
        message=(
          "This group is being referenced by other groups: %s." % ", ".join(grs)
        ),
        details={"groups": grs},
      )
    entity.key.delete()
...
class ReplicationHandler(handler.AuthenticatingHandler):
  ...
  def post(self):
    ...
    expected_ident = (
      ...
      model.Identity(
        model.IDENTITY_USER, primary_id + "@appspot.gserviceaccount.com"
      ),
    )
    ...
    try:
      ...
      logging.error(
        "bad AuthDB from %s at rev %d: %s",
        request.revision.primary_id,
        request.revision.auth_db_rev,
        e,
      )
      self.send_error(replication_pb2.ReplicationPushResponse.BAD_REQUEST)
      return
  ...
  api_doc = [
    {
      "verb": "GET",
      "params": "identity=...",
      "doc": "Returns a list of groups an identity belongs to (including all "
    ...
    {
      "verb": "POST",
      "doc": "A batch version of the membership listing call. Executes multiple "
      "queries for multiple identities in parallel.",
      "request_type": {
        "name": "Batch listing request",
        "doc": "A request to query a list of groups of multiple identities in "
      ...
      "response_type": {
        ...
        "doc": "For each identity specifies a list of groups it is a member of "
  ...
  api_doc = [
    {
      ...
      "doc": "Checks whether a user belongs to any of given groups (provided via "
      '"groups" query parameter that can be specified multiple times).',
      "response_type": {
        "name": "Check response",
        "doc": "Indicates whether the identity is a member of any of the groups "
    ...
    {
      "verb": "POST",
      "doc": "A batch version of the membership check call. Executes multiple "
      "checks for multiple identities in parallel.",
      "request_type": {
        "name": "Batch check request",
        "doc": "Represents a request to check memberships of multiple identities in "
        "parallel.",
        "example": {
          "per_identity": {
            "user:someone@example.com": {
      ...
      "response_type": {
        ...
        "doc": "For each queried identity specifies whether it is a member of any "
