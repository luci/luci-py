...
class ApiHandlerClassTest(test_case.TestCase):
  ...
  def test_parse_body_not_dict(self):
    ...
    call_post(
      Handler, "[]", content_type="application/json; charset=utf-8", status=400
    )
  ...
  def test_parse_body_bad_encoding(self):
    ...
    call_post(
      Handler, "[]", content_type="application/json; charset=ascii", status=400
    )
...
class RestAPITestCase(test_case.TestCase):
  ...
  def tearDown(self):
    ...
    try:
      self.assertEqual(
        self._initial_auth_db_rev + self._auth_db_rev_inc, get_auth_db_rev()
      )
    finally:
    ...
    self.assertEqual(
      {
        ...
        + [
          ...
          {
            "caller_can_modify": False,
            "created_by": None,
            "created_ts": 1300000000000000,
            "description": "",
            "modified_by": None,
            "modified_ts": 1300000000000000,
            "name": "z-external/group",
            "owners": "administrators",
          },
        ],
      },
...
class GroupHandlerTest(RestAPITestCase):
  ...
  def test_get_missing(self):
    status, body, _ = self.get(
      path="/auth/api/v1/groups/no-such-group", expect_errors=True
    )
  ...
  def test_get_is_using_cache(self):
    ...
    status, body, _ = self.get(
      path="/auth/api/v1/groups/a-group", headers={"Cache-Control": "no-cache"}
    )
    self.assertEqual(200, status)
  ...
  def test_get_requires_admin_or_access(self):
    ...
    self.mock_is_admin(False)
    status, body, _ = self.get(
      path="/auth/api/v1/groups/a-group", expect_errors=True
    )
    self.assertEqual(403, status)
    self.assertEqual({"text": "Access is denied."}, body)
    ...
    expected.update(
      {
        ...
        "auth_db_rev": 1,
        "auth_db_app_version": "v1a",
        "auth_db_deleted": True,
        "auth_db_change_comment": "REST API",
  ...
  def test_delete_referenced_group(self):
    ...
    self.assertEqual(
      {
        "text": "This group is being referenced by other groups: another-group.",
        "details": {
  ...
  def test_delete_by_owner(self):
    ...
    self.assertEqual(
      body,
      {"text": '"user:a@a.com" has no permission to delete group "a-group"'},
    )
  ...
  def test_delete_admins_fails(self):
    ...
    self.assertEqual(
      {
        "text": "Can't delete 'administrators' group.",
        "details": None,
      },
      body,
    )
  ...
  def test_post_bad_body(self):
    ...
    self.assertEqual(
      {
        "text": "Expecting a list or tuple for 'members', got 'dict' instead",
      },
      body,
    )
  ...
  def test_post_missing_nested(self):
    ...
    self.assertEqual(
      {
        "text": "Some referenced groups don't exist: Missing group.",
        "details": {
  ...
  def test_post_by_non_admin_with_access(self):
    ...
    self.assertEqual(403, status)
    self.assertEqual(
      body, {"text": '"user:a@a.com" has no permission to create a group'}
    )
  ...
  def test_put_missing_owners(self):
    ...
    self.assertEqual(
      {
        "text": "Owners groups (Missing group) doesn't exist.",
        "details": {"missing": ["Missing group"]},
      },
      body,
    )
  ...
  def test_put_changing_admin_owners(self):
    ...
    self.assertEqual(
      {
        "text": "Can't change owner of 'administrators' group.",
        "details": None,
      },
      body,
    )
  ...
  def test_put_dependency_cycle(self):
    ...
    self.assertEqual(
      {
        "text": "Groups can not have cyclic dependencies: a-group -> a-group.",
        "details": {"cycle": ["a-group", "a-group"]},
      },
      body,
    )
  ...
  def test_put_by_owner(self):
    ...
    self.assertEqual(
      body,
      {"text": '"user:a@a.com" has no permission to update group "a-group"'},
    )
...
class IPWhitelistHandlerTest(RestAPITestCase):
  ...
  def test_get_missing(self):
    status, body, _ = self.get(
      path="/auth/api/v1/ip_allowlists/some_whitelist", expect_errors=True
    )
    self.assertEqual(404, status)
  ...
  def test_get_requires_admin(self):
    self.mock_is_admin(False)
    status, body, _ = self.get(
      path="/auth/api/v1/ip_allowlists/bots", expect_errors=True
    )
    self.assertEqual(403, status)
    self.assertEqual({"text": "Access is denied."}, body)
  ...
  def test_post_bad_body(self):
    ...
    self.assertEqual(
      {
        "text": "u'not a subnet' is not an IP address (not IPv4 or IPv6 address)",
      },
      body,
    )
...
class MembershipsListHandlerTest(RestAPITestCase):
  ...
  def test_get_ok(self):
    ...
    self.assertEqual(200, status)
    self.assertEqual({"memberships": [{"group": "a"}, {"group": "b"}]}, body)
  ...
  def test_get_empty(self):
    ...
    self.assertEqual(200, status)
    self.assertEqual({"memberships": []}, body)
  ...
  def test_post_ok(self):
    ...
    self.assertEqual(
      {
        "per_identity": {
          "user:a@example.com": {"memberships": [{"group": "a"}]},
          "user:b@example.com": {"memberships": [{"group": "b"}]},
          "user:c@example.com": {
            "memberships": [{"group": "a"}, {"group": "b"}],
          },
          "user:d@example.com": {"memberships": []},
        },
      },
      body,
    )
...
class MembershipsCheckHandlerTest(RestAPITestCase):
  ...
  def test_get_ok(self):
    ...
    self.assertEqual(200, status)
    self.assertEqual({"is_member": True}, body)
  ...
  def test_post_ok(self):
    ...
    self.assertEqual(
      {
        "per_identity": {
          "user:a@example.com": {"is_member": True},
          "user:b@example.com": {"is_member": False},
          "user:c@example.com": {"is_member": True},
          "user:d@example.com": {"is_member": False},
        },
      },
      body,
    )
...
class SubgraphHandlerTest(RestAPITestCase):
  ...
  def test_no_arg(self):
    ...
    self.assertEqual(400, status)
    self.assertEqual({"text": "Bad principal - Not a valid group name"}, body)
    ...
    self.assertEqual(
      {
        "subgraph": {
          "nodes": [
            {"kind": "IDENTITY", "value": "user:a@example.com"},
          ],
        },
      },
      body,
    ...
    self.assertEqual(
      {
        "subgraph": {
          "nodes": [
            {"kind": "GLOB", "value": "user:*@example.com"},
          ],
        },
      },
      body,
  ...
  def test_empty_reply_group(self):
    ...
    self.assertEqual(404, status)
    self.assertEqual(
      {"text": 'The requested group "group" was not found.'}, body
    )
    ...
    self.assertEqual(
      {
        "subgraph": {
          "nodes": [
            {"edges": {"IN": [1]}, "kind": "GROUP", "value": "b-inner"},
            {"edges": {"OWNS": [2]}, "kind": "GROUP", "value": "a-root"},
            {"edges": {"IN": [3]}, "kind": "GROUP", "value": "c-owned-by-root"},
            {"edges": {"OWNS": [4]}, "kind": "GROUP", "value": "d-inc-owned"},
            {"kind": "GROUP", "value": "e-owned-by-3"},
          ]
        }
      },
      body,
...
class GroupsSuggestHandlerTest(RestAPITestCase):
  ...
  def test_get_some(self):
    ...
    self.assertEqual(200, status)
    self.assertEqual({"names": ["abc", "ade", "administrators"]}, body)
  ...
  def test_get_all(self):
    ...
    self.assertEqual(200, status)
    self.assertEqual({"names": ["abc", "ade", "administrators", "z"]}, body)
...
class OAuthConfigHandlerTest(RestAPITestCase):
  ...
  def test_post_works(self):
    ...
    copy_in_history = ndb.Key(
      "AuthGlobalConfigHistory", "root", parent=model.historical_revision_key(1)
    )
    expected = {
      "auth_db_app_version": "v1a",
      "auth_db_change_comment": "REST API",
...
class RedirectUiOnReplicaTest(test_case.TestCase):
  ...
  def test_redirects_on_replica(self):
    ...
    self.assertEqual(0, len(calls))
    self.assertEqual(
      "http://localhost:1234/some/method?arg=1", response.headers["Location"]
    )
