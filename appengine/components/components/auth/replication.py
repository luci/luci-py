...
def new_auth_db_snapshot():
  ...
  snapshot = AuthDBSnapshot(
    ...
    ip_whitelists,
    ip_whitelist_assignments,
    realms_globals_future.get_result()
...
def push_auth_db(revision, auth_db):
  ...
  state = model.get_replication_state()
  if (
    state.primary_id != revision.primary_id
    or state.auth_db_rev >= revision.auth_db_rev
  ):
    return False, state
  ...
  shard_ids = store_sharded_auth_db(
    auth_db, state.primary_url, revision.auth_db_rev, 512 * 1024
  )
