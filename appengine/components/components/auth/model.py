...
class Identity(
  ...
  def __new__(cls, kind, name):
    ...
    if kind not in ALLOWED_IDENTITY_KINDS or not ALLOWED_IDENTITY_KINDS[
      kind
    ].match(name):
...
class _AuthDBTransaction(object):
  ...
  def commit(self):
    assert not self.committed
    puts = [
      c.entity.make_historical_copy(c.deletion, c.comment) for c in self.changes
    ]
...
class AuthGroup(
  ndb.Model, AuthVersionedEntityMixin, datastore_utils.SerializableModelMixin
):
...
def find_referencing_groups(group):
  ...
  nesting_groups = AuthGroup.query(
    AuthGroup.nested == group, ancestor=root_key()
  ).fetch_async(keys_only=True)
  owned_groups = AuthGroup.query(
    AuthGroup.owners == group, ancestor=root_key()
  ).fetch_async(keys_only=True)
  refs = set()
...
def bootstrap_ip_whitelist_assignment(identity, ip_whitelist, comment=""):
  ...
  """
  entity = ip_whitelist_assignments_key().get() or AuthIPWhitelistAssignments(
    key=ip_whitelist_assignments_key()
  )
...
def fetch_ip_whitelists():
  ...
  assignments = assign_fut.get_result() or AuthIPWhitelistAssignments(
    key=ip_whitelist_assignments_key()
  )
