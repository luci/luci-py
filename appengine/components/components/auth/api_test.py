...
class AuthDBTest(test_case.TestCase):
  ...
  def test_is_group_member(self):
    ...
    is_member = lambda groups, ident, group: new_auth_db(
      groups=groups
    ).is_group_member(group, ident)
  ...
  def run_auth_db_fetch_test(self, setup_cb):
    ...
    self.assertEqual(
      {model.Anonymous: "some ip whitelist"}, auth_db._ip_whitelist_assignments
    )
    self.assertEqual(
...
class TestAuthDBCache(test_case.TestCase):
  ...
  def set_fetched_auth_db(self, auth_db):
    ...
    def mock_fetch_auth_db(known_auth_db=None):
      if (
        known_auth_db is not None
        and auth_db.auth_db_rev == known_auth_db.auth_db_rev
      ):
...
class PermissionsTest(test_case.TestCase):
  ...
  def test_validation_errors(self):
    with self.assertRaises(TypeError):
      api.Permission(123)
    with self.assertRaises(TypeError):
      api.Permission("no.unicode.here")
...
class RealmsTest(test_case.TestCase):
    ...
    return api.AuthDB.from_proto(
      ...
      auth_db=replication_pb2.AuthDB(
        ...
        realms={
          ...
          "permissions": [{"name": p.name} for p in ALL_PERMS],
          "realms": [
            {
              "name": name,
              "bindings": [
                {
                  "conditions": conds,
                  "permissions": [ALL_PERMS.index(p) for p in perms],
                  "principals": [
  ...
  def assert_check(self, db, perm, realms, ident, attrs, outcome):
    self.assertEqual(
      outcome,
      db.has_permission(perm, realms, ident, attributes=attrs),
  ...
  def test_conditional_bindings(self):
    ...
    self.assert_check(
      db, PERM1, ["p:r"], ID1_UPPER, {"a1": "a", "a2": "c"}, True
    )
    ...
    self.assert_check(
      db, PERM2, ["p:r"], ID1_UPPER, {"a1": "a", "a3": "c"}, False
    )
  ...
  def test_has_permission_dryrun(self):
    ...
    api.has_permission_dryrun(
      PERM0, ["proj:@root"], True, ID1, admin_group="admin", tracking_bug="bug"
    )
    self.assert_logs(
      "info",
    ...
    self.logs["info"] = []
    api.has_permission_dryrun(
      PERM1, ["proj:@root"], False, ID1, admin_group="admin", tracking_bug="bug"
    )
    self.assert_logs(
      "info",
    ...
    self.logs["warning"] = []
    api.has_permission_dryrun(
      PERM0, ["proj:@root"], False, ID1, admin_group="admin", tracking_bug="bug"
    )
    self.assert_logs(
      "warning",
      "bug: has_permission_dryrun('luci.dev.testing0', ['proj:@root'], "
    ...
    self.logs["warning"] = []
    api.has_permission_dryrun(
      PERM1, ["proj:@root"], True, ID1, admin_group="admin", tracking_bug="bug"
    )
    self.assert_logs(
      "warning",
      "bug: has_permission_dryrun('luci.dev.testing1', ['proj:@root'], "
    ...
    self.logs["exception"] = []
    api.has_permission_dryrun(
      PERM1, ["@root"], True, ID1, admin_group="admin", tracking_bug="bug"
    )
    self.assert_logs(
      "exception",
