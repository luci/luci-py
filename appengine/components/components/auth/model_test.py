...
class IdentityTest(test_case.TestCase):
  ...
  def test_validation(self):
    ...
    ok_identities = (
      ...
      (model.IDENTITY_USER, "joe@example.com"),
...
class IdentityGlobTest(test_case.TestCase):
  ...
  def test_validation(self):
    ...
    ok_globs = (
      ...
      (model.IDENTITY_USER, "*@example.com"),
    )
...
class FindGroupReferencesTest(test_case.TestCase):
  ...
  def test_referenced_as_nested_group(self):
    ...
    self.assertEqual(
      set(["Group 2", "Group 4"]), model.find_referencing_groups("Referenced")
    )
  ...
  def test_referenced_as_owner(self):
    ...
    self.assertEqual(
      set(["Group 1", "Group 2"]), model.find_referencing_groups("Referenced")
    )
    ...
    self.assertEqual(
      {
        ...
        "modified_by": model.get_service_self_identity(),
        "modified_ts": mocked_now,
        "subnets": ["192.168.0.0/24", "127.0.0.1/32"],
      },
      ent.to_dict(),
    )
...
class AuditLogTest(test_case.TestCase):
  ...
  def grab_log(self, original_cls):
    copies = (
      original_cls.get_historical_copy_class()
      .query(ancestor=model.root_key())
      .fetch()
    )
    # All keys under correct historical_revision_key().
    for c in copies:
      self.assertEqual(
        ndb.Key("Rev", c.auth_db_rev, parent=model.root_key()), c.key.parent()
      )
    ...
    self.assertEqual(
      {
        cpy(1): {
          ...
          "auth_db_change_comment": "Comment",
          "oauth_additional_client_ids": [],
          "oauth_client_id": "1",
          "oauth_client_secret": "",
  ...
  def test_realms_globals_log(self):
    ...
    cpy = lambda rev: ndb.Key(
      "Rev", rev, "AuthRealmsGlobalsHistory", "globals", parent=model.root_key()
    )
    self.assertEqual(
      {
        cpy(1): {
