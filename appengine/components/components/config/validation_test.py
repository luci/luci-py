...
class ValidationTestCase(test_case.TestCase):
  ...
  def test_rule(self):
    ...
    rule = validation.rule(
      "projects/foo", "bar.cfg", test_config_pb2.Config, rule_set=self.rule_set
    )
  ...
  def test_patterns(self):
    ...
    validation.rule(
      "services/foo", "foo.cfg", test_config_pb2.Config, rule_set=self.rule_set
    )
    validation.rule(
      "services/foo", "foo.cfg", test_config_pb2.Config, rule_set=self.rule_set
    )
  ...
  def test_ref_config_rule(self):
    ...
    self.assertTrue(
      self.rule_set.is_defined_for("projects/baz/refs/heads/main", "bar.cfg")
    )

    self.assertFalse(
      self.rule_set.is_defined_for("projects/baz/refs/heads/main", "nonbar.cfg")
    )
