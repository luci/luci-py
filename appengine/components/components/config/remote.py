...
class Provider(object):
  ...
  def _config_v2_client(self):
    ...
    return client.Client(
      self.service_hostname, config_service_prpc_pb2.ConfigsServiceDescription
    )
      ...
      try:
        ...
        if blob and headers.get("content-encoding", "").lower() == "gzip":
          res.raw_content = gzip.GzipFile(
            fileobj=cStringIO.StringIO(blob)
          ).read()
        else:
    ...
    if self._is_v1_host():
      ...
      res = yield self._get_config_v2_async(
        config_service_pb2.GetConfigRequest(
          config_set=config_set,
          path=path,
          fields=field_mask_pb2.FieldMask(paths=["revision", "content_sha256"]),
        )
      )
      revision = res.revision if res else revision
  ...
  @ndb.tasklet
  def get_async(
    self, config_set, path, revision=None, dest_type=None, store_last_good=None
  ):
  ...
  def _get_configs_multi(self, cfg_path):
    ...
    if self._is_v1_host():
      res = yield self._api_call_async(
        format_url("configs/projects/%s", cfg_path),
      ...
      try:
        ...
        logging.error(
          "RpcError for GetProjectConfigs(%s): %s\n" % (cfg_path, rpce)
        )
        raise rpce
    ...
    if self._is_v1_host():
      ...
      try:
        ...
        if rpce.status_code == codes.StatusCode.NOT_FOUND:
          ...
          raise ndb.Return(None)
        logging.error(
          "RpcError in getting config_set(%s) location: %s\n"
          % (config_set, rpce)
        )
        raise rpce
  ...
  def _update_last_good_config_async(self, config_key):
    ...
    def update():
      ...
      logging.info(
        "Updated last good config %s to %s", config_key.id(), revision
      )
...
def _get_last_good_async(config_set, path, dest_type):
  ...
  if dest_type and issubclass(dest_type, protobuf.message.Message):
    ...
    try:
      ...
      logging.exception(
        "Recompile %s proto message with the latest protoc", proto_message_name
      )
      proto_message_name = None
  ...
  if (
    not last_good
    or not last_good.last_access_ts
    or _maybe_update_last_access_ts(now - last_good.last_access_ts)
    or last_good.proto_message_name != proto_message_name
  ):
