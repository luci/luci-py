...
class NetTest(test_case.TestCase):
      ...
      if expected_authorization:
        ...
        if pattern_offset != -1:
          self.assertRegexpMatches(
            provided_authorization, expected_authorization[pattern_offset + 1 :]
          )
        else:
    ...
    self.mock_urlfetch(
      [
        (
          {
            "deadline": 10,
            "headers": {"Authorization": "Bearer project-id-token"},
            "method": "GET",
  ...
  def test_308_expected(self):
    self.mock_urlfetch(
      [
        ({"url": "http://localhost/123"}, Response(308, "incomplete", {})),
      ]
    )
    ...
    self.mock_urlfetch(
      [
        (
          ...
          Response(
            200, ')]}\'\n{"a":"b"}', {"example-header": "example-value"}
          ),
        ),
      ]
    )
