...
def is_valid_package_name(package_name):
  ...
  return (
    package_name
    and bool(PACKAGE_NAME_RE.match(package_name))
    and all(c.replace(".", "") != "" for c in package_name.split("/"))
  )
...
def is_valid_package_name_template(template):
  ...
  return (
    template
    and template.count("${") == template.count("{") == template.count("}")
    and all(PACKAGE_NAME_TEMPLATE_RE.match(p) for p in parts if p)
  )
...
def is_valid_version(version):
  ...
  return bool(
    is_valid_instance_id(version)
    or is_valid_tag(version)
    or REF_RE.match(version)
  )
