...
class PRPCServerTestCaseBase(test_case.TestCase):
  ...
  def get_post_response(
    self, app, url, data=None, headers="", expect_errors=False
  ):
  ...
  def check_servicer_persistence(self):
    ...
    resp = self.get_post_response(
      self.app, "/prpc/test.Test/Give", req.SerializeToString(), headers
    )
  ...
  def check_bad_app(self):
    ...
    self.assertEqual(
      self.get_response_status(resp), httplib.INTERNAL_SERVER_ERROR
    )
  ...
  def check_options_no_cors(self):
    ...
    no_cors_resp = self.get_options_response(
      self.no_cors_app, "/prpc/test.Test/Give", headers=options_headers
    )
  ...
  def check_options_allowed_origins(self):
    ...
    allowed_resp = self.get_options_response(
      self.explicit_origins_app, "/prpc/test.Test/Give", headers=options_headers
    )
    allowed_headers = allowed_resp.headers
    self.assertEqual(
      allowed_headers["Access-Control-Allow-Origin"], allowed_origin
    )
    ...
    self.assertEqual(
      allowed_headers["Access-Control-Allow-Methods"], "OPTIONS, POST"
    )
    ...
    blocked_resp = self.get_options_response(
      self.explicit_origins_app, "/prpc/test.Test/Give", headers=options_headers
    )
  ...
  def check_options_allow_cors(self):
    ...
    cors_resp = self.get_options_response(
      self.app, "/prpc/test.Test/Give", headers=options_headers
    )
    ...
    self.assertEqual(
      cors_headers["Access-Control-Allow-Methods"], "OPTIONS, POST"
    )
...
class PRPCFlaskServerTestCase(PRPCServerTestCaseBase):
  def setUp(self):
    ...
    for route in explicit_origins_s_routes:
      self.explicit_origins_app.add_url_rule(
        route[0], view_func=route[1], methods=route[2]
      )
