...
class Ereporter2Test(test_case.TestCase):
  ...
  def assertContent(self, message):
    self.assertEqual("no_reply@sample-app.appspotmail.com", message.sender)
    self.assertEqual(
      'Exceptions on "sample-app" at "2014-06-24"', message.subject
    )
    expected_html = (
  ...
  def test_email_recipients(self):
    ...
    message = messages[0]
    self.assertEqual("joe@example.com", message.to)
    self.assertContent(message)
