...
def _roll_splits(duration, starting_split=None):
  ...
  while True:
    ...
    new_weight = min(1, max(potential_weight, 0.01))
    if new_weight >= 1:
      break

    yield 1.0 - new_weight, new_weight

    duration_left = end - now
    to_sleep = min(interval, duration_left)
    print(
      "sleeping %ss for next split (%.1fs left in migration)\n"
...
def find_app_yamls(app_dir):
  ...
  if not yamls:
    raise ValueError(
      "Not a GAE application directory, no service *.yaml's found: %s" % app_dir
    )
...
def expand_files_with_luci_gae_vars(mods, app_id):
  ...
  for m in mods:
    ...
    if modified_body == m.data:
      ...
      new_body_text = json.dumps(
        modified_body, sort_keys=True, indent=2, separators=(",", ": ")
      )
...
class Application(object):
  ...
  def __init__(self, app_dir, app_id=None, verbose=False, gae_sdk=None):
    ...
    if not yaml:
      raise Error(
        "Python 2 code needs to call setup_gae_sdk before "
    ...
    if not self.app_id:
      raise ValueError(
        "application ID is neither specified in default "
  ...
  def run_cmd(self, cmd, cwd=None, env=None):
    ...
    proc = subprocess.Popen(
      cmd, cwd=cwd or self._app_dir, stdout=subprocess.PIPE, env=env
    )
  ...
  def _get_gcloud_env(self):
    ...
    if removed:
      logging.debug(
        "Stripped environment variables to avoid enterprise proxy crashes: %s",
        ", ".join(removed),
      )
    return env
  ...
  def set_default_version(self, version, services=None, roll_duration=None):
    ...
    url = "https://console.cloud.google.com/logs/viewer?" + urlencode(
      {
        "project": self.app_id,
        "minLogLevel": 0,
        "customFacets": "",
    ...
    if roll_duration and from_version != version:
      ...
      print(
        "Beginning migration, press ctrl-C to cancel and reset to %r"
        % (from_version,)
      )
  ...
  def get_uploaded_versions(self, services=None):
    ...
    actual_versions = [
      version
      for version, services_with_it in versions.items()
