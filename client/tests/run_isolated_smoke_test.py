...
CONTENTS = {
  "file1.txt": b"File1\n",
  "repeated_files.py": textwrap.dedent("""
  ...
  "max_path.py": textwrap.dedent("""
      ...
      with open(prefix + path, 'rb') as f:
        ...
        if actual != b'File1\\n':
          ...
          sys.exit(1)
      print('Success')""").encode(),
  "output.py": textwrap.dedent(
    """
      import sys
      with open(sys.argv[1], 'w') as fh:
        fh.writelines(['{}'])""".format(OUTPUT_CONTENT)
  ).encode(),
}
...
class RunIsolatedTest(unittest.TestCase):
  def setUp(self):
    ...
    self.tempdir = run_isolated.make_temp_dir(
      "run_isolated_smoke_test", test_env.CLIENT_DIR
    )
  ...
  def assertEndsWith(self, expected_ending, output):
    ...
    self.assertRegex(
      output, re.compile("(\\s|\\S)*%s$" % re.escape(expected_ending))
    )
  ...
  def test_isolated_max_path(self):
    ...
    cas_digest = self._fakecas.archive_files(
      {
        os.path.join("a" * 200, "b" * 200): CONTENTS["file1.txt"],
        "max_path.py": CONTENTS["max_path.py"],
      }
    )
    expected = [
      "state.json",
  ...
  def test_isolated_corrupted_cache_entry_different_size(self):
    ...
    cached_file_path = self._test_corruption_common(
      CONTENTS["file1.txt"] + b" now invalid size"
    )
  ...
  def test_named_cache(self):
    ...
    self.assertEqual(
      ["hello"], list_files_tree(os.path.join(self._named_cache_dir, rel_path))
    )
  ...
  def test_cas_output(self):
    ...
    self.assertEqual(
      OUTPUT_CONTENT.encode(), read_content(os.path.join(output_dir, "foo.txt"))
    )
