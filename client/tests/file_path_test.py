...
class FilePathTest(auto_stub.TestCase):
  ...
  def assertFileMode(self, filepath, mode, umask=None):
    ...
    self.assertEqual(
      expected, actual, (filepath, oct(expected), oct(actual), oct(umask))
    )
  ...
  if sys.platform == "win32":
    ...
    def test_rmtree_outliving_processes(self):
      ...
      try:
        ...
        expected_sleeps = (
          [4, 6, 8, 10, 2] if sys.platform == "win32" else [4, 6, 2]
        )
  ...
  def _check_get_recursive_size(self, symlink="symlink"):
    ...
    if symlink == "symlink":
      ...
      assert False, (
        "symlink should be one of symlink, "
  ...
  def test_scan_large_directory(self):
    ...
    self.assertEqual(
      file_path._get_recur_size_with_scandir(parent_dir), (1, 1, 1, 0, 0)
    )
