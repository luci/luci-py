...
class TestLuciContext(unittest.TestCase):
  ...
  def tearDown(self):
    self.assertNotIn(
      self.ek,
      os.environ,
      "%s in environ (%r)! Possible leak in test?"
  ...
  def test_ok(self):
    ...
    self.assertFalse(
      luci_context._check_ok({"hi": "there", "ok": {"thing": "true"}})
    )
  ...
  def test_initial_load_win(self):
    with luci_context.write(something={"data": True}):
      self.assertDictEqual(
        luci_context.read_full(), {"something": {"data": True}}
      )
      self.assertDictEqual(
        luci_context._CUR_CONTEXT, {"something": {"data": True}}
      )
