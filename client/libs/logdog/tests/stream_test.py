...
class StreamClientTestCase(unittest.TestCase):
  ...
  def testTextStream(self):
    ...
    with client.text("mystream") as fd:
      self.assertEqual(
        fd.path, streamname.StreamPath(prefix="foo/bar", name="mystream")
      )
      self.assertEqual(
        fd.get_viewer_url(),
  ...
  def testTextStreamWithParams(self):
    client = self._registry.create("test:value")
    with client.text(
      "mystream", content_type="foo/bar", tags={"foo": "bar", "baz": "qux"}
    ) as fd:
      self.assertEqual(
        fd.params,
  ...
  def testDatagramStream(self):
    ...
    self.assertEqual(
      list(self._split_datagrams(data)), [b"datagram0", b"dg1", b"", b"dg3"]
    )
