...
if sys.platform == "win32":
      ...
      if err == 1314:
        raise WindowsError(
          (
            ...
            "calling file_path.enable_symlink() first"
          )
          % (source, link_name)
        )
      raise WindowsError(
        "symlink(%r, %r) failed: %s" % (source, link_name, err)
      )
  ...
  def remove(path):
    ...
    if isdir(path):
      if not RemoveDirectory(path):
        # pylint: disable=undefined-variable
        raise WindowsError(
          "unlink(%r): could not remove directory: %s"
      ...
      if DeleteFile(path):
        return
      logging.warning(
        "failed to call DeleteFile(%r): win error: %s",
        path,
        ctypes.GetLastError(),
      )
  ...
  def readlink(path):
    ...
    if handle == INVALID_HANDLE_VALUE:
      # pylint: disable=undefined-variable
      raise WindowsError(
        "readlink(%r): failed to open: %s" % (path, ctypes.GetLastError())
      )
    try:
      ...
      if buf.ReparseTag == IO_REPARSE_TAG_SYMLINK:
        ...
        raise WindowsError(  # pylint: disable=undefined-variable
          "readlink(%r): succeeded but doesn't know how to parse result!" % path
        )
